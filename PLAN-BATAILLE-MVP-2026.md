...
- [x] **B2g** Planning intégrable — widget/iframe pour les sites des profs (demande Manon) — FAIT 2026-07-26 (commits 212ec7a→8746463, 0 agent, 0 migration). **Architecture Calendly** (l'option propre parmi 3 étudiées : iframer le portail = cookies tiers morts + chrome dupliqué ; widget DOM-injecté = collisions CSS/XSS/support infini) : route **`/embed/[slug]`** HORS de `/p/` (le layout portail ne s'hérite pas dans l'iframe), vue ANONYME seule (cours publics, `filterCoursVisibles(null)` — le cours privé du seed n'apparaît pas, prouvé), l'ACTION sort de l'iframe (chaque cours → portail en nouvel onglet `?src=embed`, tracking façon QR), jauge v74, tags prix mixte B2f, vitrine Essentiel (« Voir » sans « Réserver », prouvé par flip de plan + restore), pied « propulsé par IziSolo » (acquisition), noindex, `?semaines=`/`?type=`. **Headers chirurgicaux** : XFO SAMEORIGIN partout SAUF `/embed/*` (lookahead négatif) + `frame-ancestors *` sur l'embed seul — prouvé aux 4 coins (embed sans XFO + CSP ; portail ET dashboard toujours protégés). **widget.js une-ligne** : `<script src=…/widget.js data-studio=X async>` — iframe créée au point de collage, origine dérivée du src du script, auto-hauteur postMessage (garde-fous origine+source+shape, bornée 120-20000), options data-semaines/type/hauteur ; iframe nue en fallback (builders sans JS). **Paramètres → Ma page** : bloc snippet copiable à côté du QR + aperçu. **Walkthrough 13/13 en page HÔTE réelle** (serveur cross-origin 8888 simulant le site de Manon, les 2 variantes côte à côte) — 3 prises : le proxy default-deny 307-redirigeait `/embed/` ET `/widget.js` vers /login (allowlist étendue) ; l'auto-hauteur mesurait `documentElement.scrollHeight` qui ne descend JAMAIS sous le viewport d'une iframe → `body.scrollHeight`, prouvé par départ forcé 1200 px → ajusté 269 px (la taille exacte du contenu) ; un `finally` qui jette masque l'erreur d'origine (builder PostgREST = thenable sans `.catch`). Zéro erreur console, démo intact. 119/119, build vert, lint 105 pile. **Restes assumés** : rendu dynamique à chaque hit (pas de cache — optimiser si le trafic embed devient réel) ; ~~pas de thème/couleurs paramétrables (v2 si demande)~~ ; premier vrai test terrain = le site de Manon.
  ...
  - **Incident annonce pleine lune (2026-08-01, Maude — « aucune n'a reçu d'email, et je n'ai plus accès à mon message ni à ma PJ »)** : la DONNÉE était parfaite (15 messages 7 h 30, texte complet, photo en Blob public — l'upload prod MARCHE) et la reproduction PROD a montré le vrai mal : (1) **emails = le digest quotidien de 16 h** (envoyée à 7 h 30, le digest n'était pas encore passé — « aucun email » = le rythme normal, pas une panne ; le pied du modal promettait « un email de notification » sans dire QUAND → texte précisé « email récapitulatif une fois par jour, vers 16 h ») ; (2) **son message était introuvable PAR ELLE** : la ligne « Annonce · 15 destinataires » ne fait que se DÉPLIER (aperçu tronqué 80 caractères SANS indice photo, destinataires « Lu/Non lu » qui ressemblent à un rapport de lecture — rien n'indiquait qu'ouvrir un NOM montre le message). Fix : bouton « **Voir le message envoyé →** » en tête du groupe déplié (ouvre la 1re conversation du batch = le message + la photo en contexte) + **préfixe 📷/📎 dans l'aperçu** même quand il y a du texte (les 2 branches pro/élève de la route conversations). Preuve : repro prod (clic header = rien, données saines) puis fix local 6/6 (préfixe, bouton, message + photo naturalWidth 192). Leçon UX : une ligne agrégée qui ressemble au message mais ne l'ouvre pas = un message « perdu » pour l'utilisatrice.
  - **Email instantané messagerie (2026-08-01, décision Colin post-incident : « on oublie cette histoire de 16 h »)** : l'élève reçoit un email **dès que la prof envoie** — annonce (toutes cibles, y compris mode groupe), message 1-à-1, message de groupe. `lib/messagerie-email.js` (`envoyerEmailsMessageInstant` + `renderEmailMessageInstant`) : objet « {studio} t'a écrit », extrait du message cité (300 c., HTML échappé), mention « 📷 N photo(s) », bouton « Lire et répondre » → espace messages, **replyTo = email de la prof** (une réponse mail lui arrive directement), pied opt-out. Garde-fous : pref `notif_prefs.message.email` (même toggle qu'avant), blacklist via `sendEmail`, **claim `emails_envoyes` type `message_instant`** (annonce → ref `{profileId}:batch:{batchId}` ; fil → ref `{profileId}:conv:{convId}:{bucket3h}` + check tranche précédente = **cooldown 3-6 h par conversation**, un échange actif ne mitraille pas), claim libéré si échec d'envoi. Envois en `after()` de Next (réponse immédiate à la prof, séquentiel ~2 req/s Resend, maxDuration 120/60). **Le digest 16 h UTC devient un FILET** : branche élève skip quiconque a un `message_instant` de ce studio < 24 h (ref préfixée profileId) et rattrape les autres — échec d'envoi, messages pré-déploiement (les 15 participantes pleine lune du 01/08 7 h 30 reçoivent bien leur email au digest du soir même) ; branche pro inchangée. Textes à jour (pied du modal cours « un email tout de suite », desc catalogue élève). Preuve smoke 9/9 : envois RÉELS Resend acceptés (destinataire `delivered@resend.dev`), cooldown (2e message → 0 nouvel email), filet dans les 2 sens, aperçu HTML rendu. ⚠️ Racine du 1er échec du smoke : `.env.local` portait `RESEND_FROM_EMAIL=no-reply@izisolo.app` (domaine JAMAIS vérifié — tout envoi local échouait depuis toujours) → aligné sur `bonjour@izisolo.fr` (la clé locale = le compte prod, domaine vérifié). Reste assumé : élève → prof toujours au digest (symétrie à trancher si Maude la demande).
  - **Aide utilisateur — analyse + 5 chantiers (2026-08-01, « je valide tout, go » Colin)** : analyse d'abord (sondes prod : **0 ticket support depuis toujours, 38 feedbacks widget dont 26/30 j = LE canal vivant, zéro trace d'usage des assistants** — rien n'était loggé, ia_usage photo quasi vide depuis juin ; coût API réel ~0,004 $/échange Haiku = argument faible, les vraies raisons = qualité/honnêteté/focus). Livré : (1) **assistants LLM SUPPRIMÉS** — 3 surfaces zombies : `api/support` (chat UI déjà « Bientôt » côté page), `api/portail/[slug]/assistant` (**route PUBLIQUE payante sans aucune UI** — bouton commenté « UX pas au point ») et la page placeholder `/assistant` + FAB commenté + `AssistantBookingButton` mort (~120 lignes) ; `lib/claude` n'a plus qu'un consommateur (photo-import) ; règle gravée : plus de feature IA sans compteur ni grounding. (2) **FAQ `/support` réécrite** — l'ancienne mentait (« factures générées automatiquement » sans Stripe SaaS, « export en bas de Paramètres » alors qu'il vit sur Élèves) ; 9 questions terrain aux vrais libellés (nav/onglets vérifiés), liens ancrés vers le guide ; section « Un bug ? Une idée ? » = widget feedback canal officiel. (3) **Guide `/aide`** — 5 parcours ancrés (#premier-cours #eleves #encaisser #pointage #page-publique) ciblés sur les frictions MESURÉES (récurrence non adoptée, drop-off élèves), chaque étape au libellé d'écran réel ; lié depuis /support, la checklist dashboard (« Guide pas à pas → », inline — pas en absolu, leçon elementFromPoint) et les emails ; `/aide` ajouté au disallow robots. (4) **Emails onboarding J+1/J+3** dans le cron `expirations` : J+1 « premier cours récurrent » (skip si cours existants), J+3 « invite tes élèves » (skip si élèves existants), fenêtres [1,3)/[3,7) sans chevauchement ni backfill, dédup claims `emails_envoyes` type `onboarding` (zéro migration), fallback email_contact→auth (pattern B1d), échec = claim libéré (pattern B1g) ; `lib/onboarding-emails.js` pur + verrou `onboarding-emails.spec.js` (15 tests). (5) Preuves chemin réel : **cron local RUN1 → J+1 réel accepté Resend (delivered@resend.dev) + claim, RUN2 → 0 renvoi** (profil test antidaté, nettoyé) — au passage RUN1 a envoyé son J+3 à Ariana (inscrite 29/07, la feature en marche) ; navigateur : /support sans assistant + FAQ neuve, /aide 5 ancres naviguables, lien checklist (captures). Restes assumés : ancre `#page-publique` non liée depuis un email (guide seulement), pas d'email J+7+ (à décider selon rétention réelle).
  - **Radar 01/08 + 2 fixes (2026-08-01, demande Colin « checke les erreurs récentes »)** : lecture des 25 erreurs/30 j (radar-erreurs.mjs + sondes prod) → 3 familles. (1) 🟠 LE bug : résa portail en **500 muet sur l'index anti-doublon** `clients_unique_nom_prenom` (posé par fix-doublons-clients.sql, étape 5) — cas réel Carmen (Maude Yoga, 29/07 : 2e email + même nom → « Erreur lors de la création du profil », Maude l'a rattrapée à la main au pointage 1 min après). Fix : `reserver` attrape le 23505 → reprise de la fiche si l'email existe (course entre 2 résas), sinon **409 honnête** (« une fiche à ce nom existe déjà… ») — JAMAIS de rattachement par nom (ce serait la fiche d'autrui, homonymes/hijack) ; même erreur métier typée `fiche_homonyme` dans `lib/essai.js` (rendue 409 côté portail visiteur ET validation prof), et la promotion liste d'attente loggue enfin son échec de création de fiche (avant : erreur NON destructurée → personne silencieusement bloquée en file à vie). (2) 🟡 18/25 lignes = cron notifs-eleves × plan free tout-ouvert (plan-guard l.52) × élèves seedés @example.com du démo (Léa/Yannick, abos fin 04-05/08 ; claim libéré à l'échec → re-envoi quotidien 08h) : **`sendEmail()` skippe les domaines réservés RFC 2606/6761** en amont (`lib/email-domaines.js`, `estEmailDeTest`, `skipped:'domaine_test'`, warn console SANS reportError — module sans dépendance car lib/email tire supabase-admin instancié au chargement) — verrou `email-domaine-test.spec.js` (17 tests). (3) 🟢 déjà réglés : izisolo.app = RESEND_FROM_EMAIL du .env.local corrigé pendant le smoke d'hier (prod jamais touchée) ; checklist_masquee = v79. **Preuve chemin réel 6/6 sur le démo** (409 + message, zéro fiche doublon, fiche existante réutilisée avec présence unique, **zéro nouvelle ligne au radar** malgré l'email de confirmation parti vers @example.com), fixtures + compte auth élève nettoyés. Restes assumés : 3 fiches « Test/Debug Cloche » du 13/07 sur le démo (cosmétique) ; le ×9 du radar s'éteint seul le 05/08.
  - **Clarté des retards + règlement du mari (2026-07-31, remontées Maude)** : (1) la section « À percevoir » de Revenus EXISTAIT mais le bouton d'une séance due RENVOYAIT au pointage du cours (« obligée de me rappeler du cours ») → **encaissement direct sur place** : modal montant prérempli + mode + **note compta**, paiement lié à la présence (v65, même liaison que le pointage), la ligne disparaît ; « + N autres » → **« Tout afficher »**. (2) **« ➕ Autre règlement »** dans le modal de règlement du pointage (visible quand la séance est déjà payée) : le cas « la cliente paie aussi pour son mari » → 2e ligne compta SANS presence_id (la séance reste couverte par le 1er encaissement — invariant vérifié), intitulé « (règlement supplémentaire) », **note compta** (« paiement mari de Marie-Pierre ») ; champ note aussi ajouté à l'encaissement normal. Preuve navigateur 7/7 (encaissé depuis Revenus avec note + ligne disparue ; 2e règlement sans presence_id + 1 seul paiement lié restant). Piège sonde re-mordu et consigné : un `throw` dans le try + `process.exit` en finally = erreur AVALÉE — toujours capturer l'erreur dans une variable et l'imprimer dans le finally.
