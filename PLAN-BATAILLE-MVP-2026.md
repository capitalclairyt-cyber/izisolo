...
- **➡️ PHASE P3 COMPLÈTE — prochaine étape : P4 Stripe & prix, AVEC Colin** (séquence scriptée : `scripts/setup-stripe-saas.mjs` en test → live → env vars Vercel → redéploiement → checkout E2E coupon Founding → webhook → Customer Portal → billing admin → emails lifecycle ; les montants et noms marketing des 2 plans se décident là).
