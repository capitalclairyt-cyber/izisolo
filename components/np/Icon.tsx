/**
 * IziSolo · Icon — set fin line-only stroke 1.4 (transposé du designer).
 * Inspirations : Lucide line variant, mais minimal pour le DS IziSolo.
 */
import * as React from 'react';

export type IconName =
  | 'home' | 'cal' | 'users' | 'me' | 'plus' | 'grid' | 'search' | 'bell'
  | 'chev' | 'chart' | 'euro' | 'settings' | 'file' | 'help' | 'bookmark'
  | 'music' | 'tag' | 'out' | 'sparkle' | 'user' | 'check';
...
export function Icon({ name, size = 20, strokeWidth = 1.4, ...rest }: IconProps) {
  return (
    <svg
