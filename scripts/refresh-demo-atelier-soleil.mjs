/**
 * REFRESH du compte démo « L'Atelier Soleil » (Camille Leroux) pour les visuels.
 * ─────────────────────────────────────────────────────────────────────────────
 * Purge les données FICTIVES du studio (préserve : profil, 2 salles, et les
 * fiches élèves liées à un vrai compte auth — Colin/Maude), puis re-seed :
 *   - 6 séries hebdo (Mat, Yoga Flow, Reformer, Barre au sol, Pilates doux,
 *     Stretching) du 27/07 au 26/09, passées pointées + futures réservées
 *   - 4 évènements : Yoga Pleine Lune 28/08 (COMPLET + 3 en liste d'attente),
 *     Yoga enfants thème Océan 26/08, Atelier Rentrée 12/09, Bain sonore 20/09
 *   - ~27 élèves (@example.com, prefs de notif email/push OFF → zéro envoi
 *     réel par les crons), anniversaires aujourd'hui (Emma) et demain (Bastien)
 *   - carnets/abos aux compteurs EXACTS vs présences liées, paiements sur 6
 *     semaines (modes minuscules !), 1 paiement en attente (16 j → cloche)
...
const soft = (step, error) => { if (error) { failures++; console.warn(`⚠️ ${step}: ${error.message}`); } };
...
console.log(`🎯 Studio : ${prof.studio_nom} (${prof.studio_slug}) — plan ${prof.plan}/${prof.stripe_subscription_status}`);
...
console.log(`🔒 Fiches préservées (compte auth lié) : ${(fichesReelles || []).map(f => f.email).join(', ') || 'aucune'}`);
...
// ═════════════════════════════════════════════════════════════════════════════
{
  ...
  console.log('👤 profil mis à jour (types + bio + tarifs affichés)');
}
...
console.log(`🎫 offres : ${offres.map(o => o.nom).join(' · ')}`);
...
console.log(`🧑‍🤝‍🧑 élèves : ${clients.length} fiches créées (+ ${(fichesReelles || []).length} préservées)`);
...
console.log(`📅 cours : ${nbCours} séances (6 séries + 4 évènements)`);
...
console.log(`🎟️ carnets/abos : ${abos.length}`);
...
console.log(`✅ présences : ${presences.length}`);
...
for (const [c, , , used] of CARNETS) {
  ...
  if (linked > used) console.warn(`⚠️ compteur carnet ${c} : ${linked} présences liées > ${used} utilisées !`);
}
...
console.log(`⏳ liste d'attente : ${laRows.length}`);
...
console.log('💬 messagerie : 6 conversations (2 non lues côté prof) + 3 réactions');
...
console.log(`🗳️ sondage : 3 créneaux, ${Object.keys(VOTES).length} votants (${voteRows.length} réponses)`);
...
console.log('📋 cas à traiter : 3 ouverts (no-show, annulation tardive, sans carnet)');
...
console.log('🔔 notifications : 2 évènementielles insérées (le reste sera dérivé à l\'ouverture de la cloche)');
...
console.log('clients:', await count('clients'), '| cours:', await count('cours'), '| présences:', await count('presences'),
  '| abos:', await count('abonnements'), '| paiements:', await count('paiements'));
console.log('conversations:', await count('conversations'), '| cas ouverts:', await count('cas_a_traiter', q => q.eq('profile_id', PROFILE_ID).is('resolu_at', null)),
...
console.log(`Pleine lune : ${occupees}/16 places occupées + ${laRows.filter(r => r.cours_id === plCours.id).length} en liste d'attente`);
// Cours futurs
const { data: futurs } = await sb.from('cours').select('id').eq('profile_id', PROFILE_ID).gte('date', TODAY);
console.log(`Cours à venir (>= ${TODAY}) : ${futurs?.length}`);
