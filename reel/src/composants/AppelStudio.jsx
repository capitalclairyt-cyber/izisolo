...
const FlecheBas = ({ progression, couleur = P.accentDeep }) => {
  const longueur = 260;
  return (
    <svg width={120} height={120} viewBox="0 0 120 150" preserveAspectRatio="xMidYMin meet" fill="none" aria-hidden="true">
      <path d="M62 6 C 40 40, 84 70, 58 108 L 58 126" stroke={couleur} strokeWidth={7} strokeLinecap="round" fill="none"
        strokeDasharray={longueur} strokeDashoffset={longueur * (1 - progression)} />
      <path d="M32 104 L 58 134 L 84 104" stroke={couleur} strokeWidth={7} strokeLinecap="round" strokeLinejoin="round" fill="none"
        opacity={progression > 0.9 ? 1 : 0} />
    </svg>
  );
};

// `palette` (2026-09-26, réels « conversation ») : la carte suit la couleur de
// fond de la variante au lieu du sable ; sans elle, rien ne change pour les
// autres réels. { fond, fond2, encre, doux, pill, pillTexte, fleche, goutte }.
const PALETTE_SABLE = { fond: P.bgFrom, fond2: P.bgTo, encre: P.ink, doux: P.inkSoft, pill: P.accent, pillTexte: '#fff', fleche: P.accentDeep, goutte: P.accent };

export const AppelStudio = ({ depuis = 0, titre = ['Tu veux voir', 'sur tes vrais cours ?'], palette }) => {
  const pal = { ...PALETTE_SABLE, ...(palette || {}) };
  ...
  return (
    <AbsoluteFill style={{ opacity: voile, background: `linear-gradient(160deg, ${pal.fond} 0%, ${pal.fond2} 100%)` }}>
      <AbsoluteFill style={{ alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginTop: -60, textAlign: 'center' }}>
          <div style={{ ...monte(6), fontFamily: FONT_DISPLAY, fontWeight: 500, fontSize: 84, lineHeight: 1.06, letterSpacing: '-0.02em',
            color: pal.encre, fontVariationSettings: '"opsz" 120, "SOFT" 30', padding: '0 80px' }}>
          ...
          <div style={{ marginTop: 70, opacity: Math.min(1, pill * 1.3), transform: `scale(${interpolate(pill, [0, 1], [0.85, 1]) * battement})`,
            background: pal.pill, color: pal.pillTexte, fontFamily: FONT_BODY, fontWeight: 700, fontSize: 46, padding: '28px 70px', borderRadius: 999,
            letterSpacing: '0.01em', boxShadow: `0 22px 54px ${pal.pill}55` }}>
            Commente STUDIO
          </div>
          <div style={{ ...monte(36), marginTop: 30, fontFamily: FONT_BODY, fontWeight: 400, fontSize: 32, lineHeight: 1.4, color: pal.doux }}>
          ...
          <div style={{ marginTop: 6 }}><FlecheBas progression={fleche} couleur={pal.fleche} /></div>
        </div>
      </AbsoluteFill>
      <div style={{ position: 'absolute', left: 90, top: 1700, opacity: 0.85 }}><Logo size={34} couleur={pal.goutte} encre={pal.encre} /></div>
    </AbsoluteFill>
  );
};
