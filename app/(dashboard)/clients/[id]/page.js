...
export default async function FicheClientPage({ params }) {
  ...
  const [
    ...
    supabase.from('presences').select('*, cours_id, cours(nom, date, heure, recurrence_id, tarif_unitaire)').eq('client_id', id).eq('profile_id', user.id).order('created_at', { ascending: false }).limit(50),
