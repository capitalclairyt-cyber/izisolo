...
export default function AbonnementsClient({ abonnements: initAbo, paiementsParAbo: initPaiements }) {
  ...
  return (
    <div className="abo-page">
      ...
      <div className="abo-header animate-fade-in">
        <div>
          <h1 className="abo-title">Carnets & abos <AideContextuelle ancre="carnets-abos" titre="Tuto : les carnets au quotidien" /></h1>
...
function AboCard({ abo, paiements }) {
  ...
  const modePrincipal = paiements[0]?.mode; // ⚠️ la colonne est `mode` (mode_paiement n'existe pas)
