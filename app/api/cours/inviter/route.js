...
export const POST = withRoute({ auth: 'active', schema: coursInviterSchema, perm: 'cours_gerer' }, async ({ auth, body }) => {
  ...
  for (const p of presences || []) {
    ...
    const t = traducteurPour(p.clients?.id);
    ...
    const magicLink = slug ? await buildPortailMagicLink({ email, studioSlug: slug, langue: t.langue }) : null;
    const lien = magicLink || (slug ? `${appUrl}/p/${slug}/connexion` : appUrl);
    // « Bonjour » plutôt que « Salut »/« Coucou » (retour Maude 2026-07-30).
    const salutation = p.clients?.prenom ? t('Bonjour {prenom}', { prenom: p.clients.prenom }) : t('Bonjour');
    ...
    try {
      ...
      await sendEmail({
        ...
        to: email,
        subject: dateFR
          ? t('Ta séance « {cours} » — {date} · {studio}', { cours: cours.nom, date: dateFR, studio })
          : t('Ta séance « {cours} » · {studio}', { cours: cours.nom, studio }),
        html: `
          <div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;max-width:560px;margin:0 auto;padding:24px;">
            <h2 style="color:#b87333;margin:0 0 6px;">${t("Une séance t'attend 🌿")}</h2>
            <p style="color:#555;margin:0 0 12px;">${salutation},</p>
            <p style="color:#555;margin:0 0 16px;">
              ${profile.prenom
                ? t("{prof} t'a réservé une place pour « {cours} ».", { prof: `<strong>${profile.prenom}</strong>`, cours: `<strong>${cours.nom}</strong>` })
                : t("Ta prof t'a réservé une place pour « {cours} ».", { cours: `<strong>${cours.nom}</strong>` })}
            </p>
            <div style="background:#faf6ef;border:1px solid #eee2cf;border-radius:12px;padding:14px 18px;margin:0 0 20px;color:#555;">
              ...
              ${heureFR ? `🕐 ${heureFR}${cours.duree_minutes ? ` (${cours.duree_minutes} ${t('min')})` : ''}<br/>` : ''}
            ...
            <div style="text-align:center;margin:24px 0;">
              <a href="${lien}" style="display:inline-block;padding:14px 28px;background:#b87333;color:white;text-decoration:none;border-radius:99px;font-weight:700;font-size:1rem;">
                ${t('Voir dans mon espace')}
              </a>
            </div>
            <p style="color:#999;margin:16px 0 0;font-size:0.8125rem;">
              ${t("Cette séance est privée : elle n'apparaît que dans ton espace. Le lien te connecte sans mot de passe et expire dans 1 heure — après, connecte-toi simplement avec ton email sur le portail.")}
            </p>
