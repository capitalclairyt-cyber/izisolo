...
- [x] Lot 1 · intervenantes + ponts 1 à 3 : LIVRÉ le 2026-09-13, v111 appliquée par Colin le jour même, prouvé **43/43 en phase complète** en vrai navigateur (trois bugs trouvés et corrigés par la phase complète : upsert sur index partiel à l'acceptation du parrainage, prénom de la propriétaire absent du portail, choix de studio ignoré en lecture seule). Reste côté Colin : faire entrer l'association de Maude (le retour qui dessine le lot 3). Choix consigné : la lecture seule au downgrade est une garde d'application (routes + écrans), pas de la base (la RLS v101 gate sur l'appartenance) ; scinder ~20 policies « for all » pour l'ancrer en SQL est un chantier à part, à ouvrir si une structure en lecture seule pose problème en vrai.
- [x] Lot 2 · l'argent (relevé, dépenses, prestations, factures v2) : LIVRÉ le 2026-09-13 (migration v112 à appliquer), prouvé 19/19 en phase dégradée en vrai navigateur (relevé recalculé à la main, PDF, Compta et son cadenas, 503 honnêtes). Choix consignés : le relevé n'est jamais stocké (recalculé, figé seulement dans la prestation validée) ; un abonnement sans prix par séance vaut 0 de CA et le relevé le dit ; la dépense née d'un relevé ne se modifie qu'en réglant ou retirant la prestation ; l'exercice d'une association suit la saison (septembre → août), celui d'un studio l'année civile ; le HT et la TVA sont facultatifs. Reste : la preuve en phase B après v112, puis les rémunérations et les relevés de l'association de Maude.
- [x] Lot 3 · vie de l'asso (bureau, adhésion, documents, AG) : LIVRÉ le 2026-09-13 (migration v113 à appliquer), prouvé 24/24 en phase dégradée en vrai navigateur (association et studio jetables : entrée et page pour l'association seule, URSSAF éteinte, cas particulier et type d'offre réservés, trésorière avec ses droits, 503 honnêtes). Choix consignés : la fonction est une étiquette qui propose des droits, jamais un troisième système ; l'adhésion vit dans SA table (jamais dans `abonnements`, jamais résolue au pointage) et se vend depuis la fiche, jamais par les tunnels ; « adhésion requise pour réserver » est une règle métier (accepter par défaut, bloquer = 403), jamais un refus de pointer ; le reçu de cotisation est la facture v84 intitulée autrement ; la convocation passe par la messagerie existante (délai rappelé, jamais imposé) ; quorum compté, jamais tranché ; pas de vote électronique, pas de signature électronique. Reste : la preuve en phase B après v113, puis l'association de Maude (bureau, adhésions de la saison, statuts, AG).
- [x] Lot 4 · gestion du studio (salles + chevauchement, analyse et marge, contrat, relevé automatique) : LIVRÉ le 2026-09-13 (migration v114 à appliquer), prouvé 25/25 en phase dégradée en vrai navigateur (analyse recalculée à la main, 403 pour une prof seule et une association, 503 honnêtes, salle refusée sans rien écrire, séance dans un lieu comme avant). Choix consignés : une salle est une ligne de `lieux` rattachée (rien à changer pour ce qui lit les lieux) ; le chevauchement est tenu par la base et vérifié avant d'écrire par son miroir pur, pour une SALLE seulement (un lieu sans salle n'est jamais bloquant) ; la marge ne fabrique aucun coût (null sans rémunération convenue) et l'analyse dit ce qui n'est rattaché à aucune séance ; le contrat est un document de la structure (v113) déposé depuis la ligne d'équipe ; le relevé automatique est opt-in, cinq jours de fenêtre, jamais un relevé vide, la validation reste à la main. Reste : la preuve en phase B après v114 ; les stats (remplissage, no-show, panier moyen) restent au lot 6.
- [x] Lot 5 · vitrines, portails croisés, hub élève, centre d'aide : LIVRÉ le 2026-09-13 (migration v115 à appliquer), prouvé 23/23 en phase dégradée en vrai navigateur (vitrines en anonyme et sur mobile, page d'intervenante, « Relier nos pages » avec ses refus, hub d'une élève de deux studios, page sans session, FAQ). Choix consignés : les vitrines ne promettent que ce que les lots 1 à 4 ont livré et disent ce que le plan ne fait pas ; relier les pages est le choix de la personne (opt-in par appartenance), jamais de la structure, et rien ne se cite vers un portail fermé ; la page d'intervenante ne montre que ses séances publiques ; le hub lit les fiches par compte puis par email et ne fait jamais traverser une donnée d'un studio à l'autre. **Le chantier est livré de bout en bout** : restent v112 à v115 à appliquer (chaque preuve bascule alors en phase complète), le setup Stripe, et l'association de Maude en vrai. Le lot 6 (stats) attend un vrai mois d'usage.
