...
export async function claimCronPush({ profileId, clientId, type, relatedId }) {
  ...
  if (error.code === '23505') return false; // doublon = déjà poussé, normal
  // Toute AUTRE erreur doit HURLER au radar : le CHECK v19 (email/sms
  // seulement) rejetait 'push' en 23514 → interprété « déjà poussé » →
  // TOUS les push de crons élèves morts en silence depuis toujours
  // (B1g, prouvé par sonde — migration v77 étend le CHECK).
  const { reportError } = await import('./report');
  reportError('[push] claimCronPush err:', error, { route: 'claimCronPush' });
  return false;
}
