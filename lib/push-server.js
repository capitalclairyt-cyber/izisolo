...
export async function sendPushToUser(userId, payload, opts = {}) {
  ...
  if (opts.type) {
    ...
    if (prof && !wantsNotif(prof.notif_prefs, opts.type, 'prof')) return 0;
  }
...
export async function sendPushToEmail(email, payload, opts = {}) {
  ...
  if (opts.type && opts.profileId) {
    const { data: c } = await supabaseAdmin
      ...
      .eq('profile_id', opts.profileId).ilike('email', clean).maybeSingle();
    if (c && !wantsNotif(c.notif_prefs, opts.type, 'eleve')) return 0;
  }
