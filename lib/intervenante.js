...
export async function lireIntervenantes(supabase, coursIds) {
  ...
  // Par LOTS : un `.in()` PostgREST tient jusqu'à ~350 ids (14 Ko d'URL) puis
  // la requête est refusée, et comme cette lecture est défensive, l'échec
  // rendait une carte vide : tous les « avec Léa » disparaissaient d'un coup
  // dès qu'un studio dépassait 350 séances à venir (mesuré le 2026-09-23 en
  // élargissant la fenêtre du portail). Même taille de lot que les photos.
  const out = {};
  try {
    for (let i = 0; i < ids.length; i += LOT_IDS) {
      const { data, error } = await supabase
        ...
        .in('id', ids.slice(i, i + LOT_IDS));
      if (error) return {};
      for (const c of data || []) if (c.intervenant_id) out[c.id] = c.intervenant_id;
    }
    return out;
  } catch {
    return {};
...
/** Taille d'un lot d'ids dans un `.in()` (≈ 37 caractères par uuid, loin du plafond d'URL). */
export const LOT_IDS = 120;
