...
// Import depuis constantes.js (source unique) + re-export pour rétrocompat
// des fichiers qui importent depuis ici. ⚠️ Un pur `export { X } from '...'`
// ne met PAS X dans le scope local — or compterSmsMois() l'utilise plus bas.
import { SMS_PRIX_UNITAIRE, SMS_ENABLED } from './constantes';
export { SMS_PRIX_UNITAIRE };
...
export async function sendNotifEleve(supabase, opts) {
  // `t` (v122) : le traducteur de l'élève, pour le pied de l'enveloppe HTML.
  // Les textes du sujet et du corps sont déjà traduits par l'appelant.
  const { profile, client, type, relatedId = null, contexte = {}, templates, prefsOverride, proEmail = null, t = traducteur('fr') } = opts;
  ...
  if (prefs.email && client.email && templates.email) {
    const result = await tryInsertAndSendEmail(supabase, {
      profile, client, type, relatedId, proEmail, t,
  ...
  // SMS_ENABLED = kill-switch GLOBAL (constantes) : sans lui, poser les clés
  // OctoPush un jour aurait réveillé les SMS des règles malgré la décision
  // produit « SMS désactivé » (B1g). Un souhait SMS non servi est TRACKÉ en
  // skipped au lieu de disparaître sans trace.
  if (prefs.sms && client.telephone && templates.sms
      && (!SMS_ENABLED || smsGlobalOff || !smsProviderConfigured)) {
    skipped++;
  }
  if (SMS_ENABLED && prefs.sms && !smsGlobalOff && client.telephone && templates.sms && smsProviderConfigured) {
...
// ─── Email via Resend ───────────────────────────────────────────────────────
async function tryInsertAndSendEmail(supabase, { profile, client, type, relatedId, sujet, corps, proEmail = null, t = traducteur('fr') }) {
  ...
  if (insertErr) {
    if (insertErr.code === '23505') {
      // Slot déjà pris — MAIS s'il correspond à un envoi ÉCHOUÉ, il était
      // consommé À VIE (l'élève ne serait JAMAIS alertée, aucun retry — B1g).
      // On re-clame uniquement les lignes 'failed', puis on renvoie.
      let q = supabase.from('notifications_eleves')
        .update({ statut: 'sent', error_message: null, sujet })
        .eq('profile_id', profile.id).eq('client_id', client.id)
        .eq('type', type).eq('channel', 'email').eq('statut', 'failed');
      q = relatedId == null ? q.is('related_id', null) : q.eq('related_id', relatedId);
      const { data: reclaimed, error: reclaimErr } = await q.select('id');
      if (reclaimErr || !reclaimed || reclaimed.length === 0) return 'skipped';
      // slot re-clamé → on tombe sur l'envoi ci-dessous
    } else {
  ...
  // Send via le pipeline central (Sprint 5) : blacklist + List-Unsubscribe + reportError
  const r = await sendEmail({
    to: client.email,
    subject: sujet,
    html: htmlWrap(corps, profile.studio_nom, t),
    replyTo: proEmail,
    categorie: 'notification',
  });
  if (r.ok) return 'sent';
  await markFailed(supabase, profile.id, client.id, type, relatedId, 'email',
    String(r.skipped || r.error || 'send failed').slice(0, 200));
  return r.skipped === 'blacklist' ? 'skipped' : 'failed';
}
...
async function tryInsertAndSendSms(supabase, { profile, client, type, relatedId, corps }) {
  ...
  if (insertErr) {
    if (insertErr.code === '23505') {
      // Même re-clame des lignes 'failed' que l'email (B1g).
      let q = supabase.from('notifications_eleves')
        .update({ statut: 'sent', error_message: null })
        .eq('profile_id', profile.id).eq('client_id', client.id)
        .eq('type', type).eq('channel', 'sms').eq('statut', 'failed');
      q = relatedId == null ? q.is('related_id', null) : q.eq('related_id', relatedId);
      const { data: reclaimed, error: reclaimErr } = await q.select('id');
      if (reclaimErr || !reclaimed || reclaimed.length === 0) return 'skipped';
    } else {
...
async function markFailed(supabase, profileId, clientId, type, relatedId, channel, errorMessage) {
  // ⚠️ .is() n'accepte que null/true/false — avec un UUID le filtre est
  // invalide et l'update ÉCHOUAIT en silence : la ligne restait 'sent' même
  // quand Resend avait échoué (audit 2026-07-25).
  let q = supabase
    .from('notifications_eleves')
...
function htmlWrap(corps, studioNom, t = traducteur('fr')) {
  ...
  return `
    <div style="font-family: -apple-system, BlinkMacSystemFont, sans-serif; max-width: 560px; margin: 0 auto; padding: 24px; color: #555; line-height: 1.6;">
      ...
      <p style="color: #aaa; font-size: 0.8rem; margin: 32px 0 0; border-top: 1px solid #eee; padding-top: 16px; text-align: center;">
        ${studioNom || t('Studio')} · ${t('Propulsé par')} <a href="https://www.izisolo.fr" style="color: #d4a0a0;">IziSolo</a>
      </p>
    </div>
  `;
}
