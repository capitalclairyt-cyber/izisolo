...
/**
 * Échappe les jokers PostgREST (`%`, `_`, `\`) pour un `.ilike()`.
 * OBLIGATOIRE sur tout `.ilike('email', …)` : sans ça, un email contenant
 * `_` (ex. jean_dupont@x.com) devient un motif joker et peut matcher la
 * MAUVAISE fiche du studio (ou casser un `.single()`).
 */
...
/**
 * Échappe le HTML d'un texte saisi par un tiers avant de l'injecter dans un
 * email ou une page. VIT ICI, et pas dans le module qui l'a vu naître
 * (lib/messagerie-email.js) : celui-là importe Supabase et Resend, donc les
 * modules PURS — ceux que les specs Node chargent sans navigateur — ne
 * pouvaient pas s'en servir et se seraient écrit une deuxième copie.
 */
...
/**
 * Normalise un texte pour la recherche : minuscules, accents retirés, trim.
 */
...
/**
 * Recherche tolérante : chaque terme de la requête (espaces = séparateurs)
 * doit apparaître quelque part dans les champs fournis, sans tenir compte de
 * la casse ni des accents. « Florence » + espace final trouve Florence,
 * « flo dup » trouve Florence Dupont, « helene » trouve Hélène.
 * (Retour Maude 2026-07-23 : « Florence␣ » ne trouvait pas Florence — la
 * requête n'était pas trimée et chaque champ était testé séparément.)
 */
export function matchRecherche(query, ...champs) {
  const q = normaliseTexte(query);
  if (!q) return true;
  const hay = normaliseTexte(champs.filter(Boolean).join(' '));
