...
import { SUPPORT_EMAIL, extraitEmail } from './messagerie-support';
import { traducteur, langueEleve } from './i18n-portail';
import { chargerLanguesFiches, chargerLanguesStudios } from './i18n-portail-serveur';
 ...
 *
 * v122 (2026-09-22) : l'email part dans la langue de l'ÉLÈVE, lue sur sa fiche
 * (`clients.langue`) puis sur le réglage du studio (`profiles.langue_portail`),
 * par les lecteurs défensifs de lib/i18n-portail-serveur (jamais dans le
 * select principal des fiches : sans les colonnes, français). Le texte du
 * message de la prof et le nom du studio viennent de la base et ne sont
 * jamais traduits.
 */
 ...
 */
export function renderEmailMessageInstant({ prenom, studioNom, contenu = '', nbPieces = 0, url, t = traducteur('fr') }) {
  const extrait = (contenu || '').trim().slice(0, 300);
  const suspension = (contenu || '').trim().length > 300 ? '…' : '';
  // Le pluriel se joue dans chaque langue : deux clés.
  const mentionPieces = nbPieces > 0
    ? (nbPieces > 1
      ? t('📷 {n} photos ou fichiers joints, à voir dans ton espace', { n: nbPieces })
      : t('📷 {n} photo ou fichier joint, à voir dans ton espace', { n: nbPieces }))
    : '';
  ...
  return `
    <div style="font-family: -apple-system, BlinkMacSystemFont, sans-serif; max-width: 560px; margin: 0 auto; padding: 24px; color: #555; line-height: 1.6;">
      <p>${bonjour},</p>
      <p>${t("{studio} t'a envoyé un message :", { studio })}</p>
      ${extrait ? `
      ...
      <p style="color: #aaa; font-size: 0.8rem; margin: 32px 0 0; border-top: 1px solid #eee; padding-top: 16px; text-align: center;">
        ${t("Tu reçois cet email dès que {studio} t'écrit. Tu peux le désactiver dans ton espace élève, réglages de notifications (section « Messages »).", { studio: escapeHtml(studioNom) })}
...
export async function envoyerEmailsMessageInstant({
  ...
  // v122 : la langue de chaque destinataire, en un lot (fiches + studio),
  // par les lecteurs défensifs : sans les colonnes, tout le monde en français.
  const languesFiches = await chargerLanguesFiches(admin, (clients || []).map(c => c.id));
  const languesStudios = await chargerLanguesStudios(admin, [profileId]);
  const studioLangue = { langue_portail: languesStudios.get(profileId) };
  ...
  for (const c of (clients || [])) {
    ...
    const r = await sendEmail({
      ...
      to: dest,
      subject: t("{studio} t'a écrit", { studio: studioNom }),
      replyTo,
      html: renderEmailMessageInstant({ prenom: c.prenom, studioNom, contenu, nbPieces, url, t }),
    });
    if (r.ok) bilan.sent++;
    else {
      bilan.failed++;
 ...
 *  `premierContact` : l'équipe INITIE (aucun message de la prof avant) —
 *  « t'a écrit », pas « t'a répondu » (dire « répondu » à quelqu'un qui n'a
 *  rien demandé sonne faux). */
export function renderEmailReponseSupport({ prenom, contenu = '', url, premierContact = false }) {
  // Teaser : 3 lignes max (demande Colin 2026-08-19) — le message complet se
  // lit dans la messagerie, l'email ramène dans l'app.
  const { texte: extrait, tronque } = extraitEmail(contenu, { maxLignes: 3, maxChars: 240 });
  return `
    <div style="font-family: -apple-system, BlinkMacSystemFont, sans-serif; max-width: 560px; margin: 0 auto; padding: 24px; color: #555; line-height: 1.6;">
      <p>Bonjour ${escapeHtml(prenom || 'là')},</p>
      <p><strong>L'équipe IziSolo</strong> ${premierContact ? 't\'a écrit' : 't\'a répondu'} 🌿</p>
      ${extrait ? `
      <blockquote style="margin: 16px 0; padding: 12px 16px; background: #faf6f3; border-left: 3px solid #b87333; border-radius: 0 8px 8px 0; color: #444; white-space: pre-line;">${escapeHtml(extrait)}${tronque ? '…' : ''}</blockquote>` : ''}
      ${tronque ? '<p style="color: #888; font-size: 0.85rem; margin: -6px 0 16px;">Le message complet t\'attend dans ta messagerie.</p>' : ''}
      <p style="text-align: center; margin: 24px 0;">
 ...
 */
export async function envoyerEmailReponseSupport({ profileId, conversationId, messageId, contenu = '', premierContact = false }) {
  ...
  const r = await sendEmail({
    ...
    to: dest,
    subject: premierContact ? 'L\'équipe IziSolo t\'a écrit 🌿' : 'L\'équipe IziSolo t\'a répondu 🌿',
