...
export async function annulerProgramme(admin, emailId) {
  ...
  if (email.resend_id) {
    ...
    // Déjà parti ou inconnu : Resend refuse. On ne ment pas : l'email reste « programmé ».
    if (error) return refus('RESEND', `Resend n'a pas pu annuler : ${error.message || JSON.stringify(error)}`, 502);
  }
