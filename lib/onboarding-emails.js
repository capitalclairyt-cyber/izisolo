...
// ----------------------------------------------------------------------------
// Trois emails, ciblés sur les frictions d'activation MESURÉES :
...
//   • J+7 « Ta première vente » (2026-08-18, état des lieux aide : silence
//     total entre J+3 et la relance de fin d'essai — pile le creux où une
//     prospect décroche) — boucle argent + invitation à RÉPONDRE (le contact
//     humain à mi-essai, celui qui a converti Patricia).
// Chacun est SKIPPÉ si l'activation correspondante a déjà eu lieu (des cours
// existent / des élèves existent / une vente existe) : on ne « relance »
// jamais quelqu'un qui a déjà fait le geste.
//
// Fenêtres SANS chevauchement : J+1 = [1 j, 3 j), J+3 = [3 j, 7 j),
// J+7 = [7 j, 10 j). Au-delà, plus rien — pas de backfill des anciens comptes
// au déploiement (les comptes > 10 j au déploiement de J+7 ne le recevront
// jamais, voulu).
// Dédup : claims `emails_envoyes` type 'onboarding', ref `${profileId}:j1|j3|j7`
 ...
 * @returns {'j1'|'j3'|'j7'|null}
 */
export function choisirEmailOnboarding({ createdAt, nbCours = 0, nbClients = 0, nbVentes = 0 }, now) {
  ...
  if (ageJours >= 7 && ageJours < 10) {
    return nbVentes > 0 ? null : 'j7'; // une vente existe déjà : geste fait
  }
  return null;
}
...
export function renderEmailOnboarding(type, { prenom = '', appUrl = 'https://www.izisolo.fr' } = {}) {
  ...
  if (type === 'j7') {
    return {
      subject: 'Ta première vente, et une vraie question',
      html: `
        <div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;max-width:560px;margin:0 auto;padding:24px;">
          <h2 style="color:#b87333;margin:0 0 6px;">Une semaine déjà !</h2>
          <p style="color:#555;margin:0 0 14px;">${bonjour}</p>
          <p style="color:#555;margin:0 0 14px;">
            Le geste qui rend IziSolo rentable : vendre ton premier carnet ou abo.
            Crée ton <strong>offre</strong> (carnet de 10, abo mensuel, annuel…), puis
            vends-la depuis la <strong>fiche de l'élève</strong> : payé maintenant, à régler
            plus tard, ou <strong>en plusieurs fois</strong>. Le suivi (qui doit quoi, quand)
            se fait tout seul dans Revenus.
          </p>
          <div style="text-align:center;margin:24px 0;">
            <a href="${appUrl}/offres/nouveau" style="display:inline-block;padding:14px 28px;background:#b87333;color:white;text-decoration:none;border-radius:99px;font-weight:700;">
              Créer ma première offre
            </a>
          </div>
          <p style="color:#555;margin:0 0 14px;font-size:0.875rem;">
            Le pas-à-pas complet :
            <a href="${appUrl}/aide#offres" style="color:#b87333;font-weight:600;">construis ton catalogue</a>.
          </p>
          <p style="color:#555;margin:20px 0 0;font-size:0.875rem;">
            Et la vraie question : <strong>qu'est-ce qui coince ?</strong> Un truc pas clair,
            un manque, une idée : réponds à cet email, je lis chaque réponse et
            je réponds moi-même.
          </p>
        </div>
      `,
    ...
    html: `
      <div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;max-width:560px;margin:0 auto;padding:24px;">
        ...
        <p style="color:#555;margin:0 0 14px;">
          ...
          (réservations, carnet, messages), inclus dans tes 30 jours de Complet.
        </p>
        <div style="text-align:center;margin:24px 0;">
