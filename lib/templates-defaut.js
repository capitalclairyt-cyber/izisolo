 ...
 * Accepte les deux formats : {{key}} (double) et {key} (simple).
 * Variables non trouvées sont laissées telles quelles.
 *
 * Permissif côté prof : peu importe ce qu'elle a tapé dans son template
 * (avec ou sans doubles accolades), on remplace toujours correctement.
 */
export function appliquerVariables(texte, contexte = {}) {
  if (!texte) return '';
  // D'abord les doubles accolades {{key}} pour éviter d'avoir un mismatch
  // (ex : "{{prenom}}" serait sinon partiellement matché par /\{(\w+)\}/).
  let out = texte.replace(/\{\{\s*(\w+)\s*\}\}/g, (match, key) => {
    return contexte[key] != null ? String(contexte[key]) : match;
  });
  // Puis les simples {key} sur ce qui reste
