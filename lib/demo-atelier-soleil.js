...
export async function refreshDemoAtelierSoleil(sb, { demoToday = null, log = console.log } = {}) {
  ...
  const jourNum = (iso) => String(Number(iso.slice(8, 10))); // « le 18 »
  // Vraie photo d'une séance pleine lune de Maude, déposée UNE fois sur le Blob
  // du démo par la route /api/messagerie/upload (2026-09-06). Stable : le seed la référence.
  const PHOTO_PLEINE_LUNE = 'https://ecacjkoszxoti9j7.public.blob.vercel-storage.com/messagerie/17a6194a-87e6-47e2-ac31-c6224cd78f44/1788677832607-pleine-lune-V9s4sagH4SKsQ7HQN2QH6veS5EPXk8.jpg';
  ...
  const mkConv = async ({ type, clientKey, coursKey, titre, messages, profLastRead, membres }) => {
    ...
    const msgRows = messages.map(m => ({
      ...
      message_type: m.photo ? 'photo' : 'text', content: m.texte, created_at: m.a,
      media_urls: m.photo ? [m.photo] : [],
    }));
  ...
  const canalPL = await mkConv({
    ...
    messages: [
      ...
      // Vraie photo d'une pleine lune de Maude (Vercel Blob du démo, déposée par
      // la route upload le 2026-09-06) : c'est elle que la landing montre.
      { de: 'pro', a: T(YEST, '19:12'), photo: PHOTO_PLEINE_LUNE, texte: `Un souvenir de la dernière pleine lune, pour vous donner envie 🌕 Il reste 2 places pour le ${jourNum(D('2026-08-28'))}, dites-le autour de vous !` },
      { de: 'sophie', a: T(YEST, '19:30'), texte: "Magnifique 😍 J'y étais, c'était magique." },
      { de: 'marion', a: T(YEST, '20:14'), texte: "Je viens avec ma sœur, je l'inscris ce soir 🙏" },
      { de: 'pro', a: T(YEST, '20:31'), texte: 'Parfait Marion, il restera une place après elle. Pensez au plaid, il fait frais après 21 h ✨' },
    ],
    profLastRead: T(TODAY, '09:02'),
    membres: [
      ['lea', T(YEST, '20:40')], ['sophie', T(YEST, '20:40')], ['emma', T(D('2026-08-10'), '22:10')],
      ['marion', T(YEST, '20:40')], ['ines'], ['margaux'], ['thomas'], ['nadia'], ['lucie'], ['elise'],
  ...
  const { error: eReact } = await sb.from('messages_reactions').insert([
    ...
    { message_id: canalPL.messageIds[2], user_type: 'eleve', user_id: C.lea.id, emoji: '🌕' },
    { message_id: canalPL.messageIds[2], user_type: 'eleve', user_id: C.emma.id, emoji: '😍' },
    { message_id: canalPL.messageIds[2], user_type: 'eleve', user_id: C.nadia.id, emoji: '🙏' },
  ]);
