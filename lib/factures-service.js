...
export async function chargerFacturation(admin, profileId) {
  // ⚠️ Les deux listes de colonnes sont écrites EN TOUTES LETTRES, jamais
  // composées par template : `scripts/verifier-selects.mjs` rejoue les selects
  // statiques contre la prod, et un `${...}` le rend aveugle à celui-ci.
  // Une vérification qu'on désarme pour économiser une ligne ne sert plus.
  //
  // Tentative complète (v105 : + le pays, qui décide du libellé imprimé).
  const { data, error } = await admin
    .from('profiles')
    .select('facturation_siret, facturation_raison_sociale, facturation_mention_tva, pays')
    .eq('id', profileId)
    .maybeSingle();
  if (!error && data) return { active: facturationActive(data), facturation: data };

  // ⚠️ Une colonne inconnue fait échouer TOUTE la requête (PGRST204), pas
  // seulement elle. Sans ce repli, déployer v105 avant d'appliquer sa
  // migration retirerait leur facture à TOUTES les profs françaises, en
  // silence, jusqu'à ce que Colin passe le SQL. On relit donc sans `pays`,
  // et la France reste le défaut — ce qu'elle était déjà pour tout le monde.
  ...
  if (repli.error || !repli.data) {
    // 42703 = migration v84 pas appliquée : facturation inactive, reçu simple.
    return { active: false, facturation: null };
  }
  const facturation = { ...repli.data, pays: 'FR' };
  return { active: facturationActive(facturation), facturation };
}
...
export async function facturesPourPaiements(admin, paiementIds) {
  ...
  for (const l of data || []) {
    // v112 : une facture non acquittée passée « payee » porte AUSSI son
    // paiement (celui que le règlement de la prestation a créé).
    if (l.facture?.statut === 'emise' || l.facture?.statut === 'payee') {
