...
export const TODO_OPS = [
  ...
  },
  {
    ...
    statut: 'a_faire',
    ajoute: '2026-08-21',
    titre: 'Finir le branchement de capsule.izisolo.fr',
    description:
      'FAIT le 21/08 : domaine ajouté sur Vercel, DNS déjà couvert par le wildcard LWS, '
      + 'hôte prouvé en prod (session admin réelle 4/4). RESTENT : (1) Supabase → Auth → '
      + 'URL Configuration → Redirect URLs : ajouter https://capsule.izisolo.fr/** '
      + '(liens magiques + reset) ; (2) installer la PWA sur les 2 téléphones (menu ⋮ → '
      + 'Installer / Partager → Sur l\'écran d\'accueil) ; (3) activer la MFA dans '
  ...
  // ── Commercial & croissance ───────────────────────────────────────────────
  {
    ...
    description:
      ...
      + 'à renseigner dans Paramètres → Argent → Facturation pour que ses clientes CSE téléchargent '
