...
function fmtPeriode(mois) {
  const [a, m] = String(mois || '').split('-');
  const noms = ['janv.', 'févr.', 'mars', 'avr.', 'mai', 'juin', 'juil.', 'août', 'sept.', 'oct.', 'nov.', 'déc.'];
  return noms[parseInt(m, 10) - 1] ? `${noms[parseInt(m, 10) - 1]} ${a}` : String(mois || '—');
}
 ...
 */
export async function genererFacturePdf({ type, numeroAffiche, dateEmission, snapshot, titre = null }) {
  const { emetteur = {}, client = {}, lignes = [], total = 0, mention_tva: mentionTva } = snapshot || {};
  const estFacture = type === 'facture';
  // v112 : une facture NON acquittée (prestation d'une intervenante adressée
  // à une structure) : même document, mais « Période » à la place de « Réglé
  // le », et « À régler à réception » à la place de l'acquittement.
  const aRegler = snapshot?.a_regler === true;
  ...
  const enTeteTableau = () => {
    ...
    page.drawText(aRegler ? 'Période' : 'Réglé le', { x: COL_DATE, y, size: 10, font: fontBold, color: NOIR });
  ...
  y = Math.min(y - 14, 706);
  // v113 : le reçu de cotisation d'une association est CE document (même
  // numérotation, même snapshot), intitulé autrement.
  page.drawText(titre || (estFacture ? 'FACTURE' : 'REÇU DE PAIEMENT'), { x: LEFT, y, size: 14, font: fontBold, color: BRAND });
  y -= 18;
  ...
  // ── Client ────────────────────────────────────────────────────────────────
  ligneTexte(estFacture ? (aRegler ? 'Adressée à :' : 'Facturée à :') : 'Émis pour :', { dy: 16 });
  ...
  if (aRegler && client.rna) ligneTexte(`RNA : ${client.rna}`);
  if (aRegler && client.siret) ligneTexte(`SIRET : ${formaterSiret(client.siret)}`);
  ...
  for (const l of lignes) {
    ...
    page.drawText(aRegler ? fmtPeriode(l.periode) : fmtDateCourte(l.date_reglement), { x: COL_DATE, y, size: 11, font, color: NOIR });
    page.drawText(aRegler ? '' : fmtMode(l.mode), { x: COL_MODE, y, size: 11, font, color: NOIR });
  ...
  y -= 24;
  if (estFacture && aRegler) {
    page.drawText('À régler à réception, par virement de préférence.', { x: LEFT, y, size: 10, font: fontBold, color: NOIR });
  } else if (estFacture) {
