...
import { dateComptable, moisComptable, montantFr, mentionExclusions } from './urssaf.js';
 ...
 */
export function construireLivreRecettes({ paiements = [], numeros = new Map(), periode, emetteur = {}, exclusions = null }) {
  ...
  return {
    ...
    // v95 : ce que la prof a demandé d'écarter (« je déclare à part »). Le
    // registre le DIT — un document qui se prétend complet en masquant des
    // lignes serait un faux ; celui-ci annonce ce qu'il ne contient pas.
    mentionExclusions: mentionExclusions(exclusions || {}),
  };
}
...
export function livreEnCsv(livre) {
  ...
  // Ce que le registre ne contient PAS, dit sur le registre lui-meme (v95).
  if (livre.mentionExclusions) out.push('', l([livre.mentionExclusions]));
  return '﻿' + out.join('\r\n');
}
