...
import { TYPES_STRUCTURE, typeStructure, formaterRna } from '@/lib/structure';
...
export const RUBRIQUES = [
  ...
  },
  {
    ...
    resume: (p, ctx) => {
      ...
      const type = typeStructure(p);
      const structure = type === 'solo' ? null : TYPES_STRUCTURE[type].label.replace(/^Une? /, '');
      return [p?.studio_nom, structure, metier, p?.ville, lieux != null ? pluriel(lieux, 'lieu', 'lieux') : null].filter(Boolean).join(' · ') || 'Nom, métier, ville et lieux';
    },
  },
  {
    id: 'equipe', groupe: 'studio', label: 'Équipe', icone: 'UserCog',
    cartes: [], lien: '/equipe', aide: 'equipe',
    // Toujours visible (lot 1 Assos & Studios) : le volet « Ailleurs » (les
    // structures où je donne cours, en faire entrer une) est un geste de la
    // PERSONNE, sans plan. L'équipe du studio, elle, reste derrière le plan.
    resume: (p) => (p && can(p, 'equipe') ? 'Inviter des profs, régler leurs droits, mes autres structures' : 'Les structures où je donne cours'),
  },
  ...
  },
  {
    ...
    cartes: ['urssaf'],
    // v113 : une association ne déclare pas de chiffre d'affaires à l'URSSAF.
    visible: (p) => aDeclarationAutomatisable(p?.pays) && typeStructure(p) !== 'association',
    resume: (p) => {
...

  {
    ...
    resume: (p) => {
      ...
      if (trial.expired) return 'Essentiel · gratuit';
...
// ── Résumés PAR CARTE (lot 2 « le repli », 2026-09-09) ─────────────────────
// Une carte fermée doit DIRE son état sur une ligne : c'est ce qui autorise le
// repli sans cacher quoi que ce soit. Mêmes règles que les résumés de
// rubrique : pur, jamais une exception, jamais un secret.
const RESUMES_CARTES = {
  profil:     (p) => resumeRubrique(rubriqueParId('profil'), p),
  activite:   (p) => [p?.studio_nom, METIERS[p?.metier]?.label, p?.ville].filter(Boolean).join(' · ') || 'Nom, métier, ville',
  structure:  (p) => {
    const type = typeStructure(p);
    if (type === 'association') return p?.rna ? `Association · RNA ${formaterRna(p.rna)}` : 'Association · RNA à renseigner';
    if (type === 'studio') return 'Studio, avec des intervenantes';
    return 'Prof à ton compte';
  },
  lieux:      (p, ctx) => {
    // v114 : une salle est une ligne de lieux rattachée (salle_de) : on ne
    // compte que les lieux.
    const l = (Array.isArray(ctx?.lieux) ? ctx.lieux : []).filter(x => !x?.salle_de);
    if (!l.length) return 'Aucun lieu pour l\'instant';
    return `${pluriel(l.length, 'lieu', 'lieux')} · ${l.map(x => x?.nom).filter(Boolean).slice(0, 3).join(', ')}${l.length > 3 ? '…' : ''}`;
  },
  facturation: (p) => resumeRubrique(rubriqueParId('facturation'), p),
  reglement:   (p) => resumeRubrique(rubriqueParId('virement'), p),
  urssaf:      (p) => resumeRubrique(rubriqueParId('urssaf'), p),
  champs:      (p) => resumeRubrique(rubriqueParId('champs'), p),
  page:        (p) => {
    const parts = [];
    if (p?.photo_couverture) parts.push('couverture');
    if (p?.photo_url) parts.push('photo');
    if (p?.bio) parts.push('bio');
    if (p?.instagram_url || p?.facebook_url || p?.website_url) parts.push('réseaux');
    return parts.length ? parts.join(' · ') : 'Couverture, photo, bio, réseaux';
  },
  page_affichage: (p) => [
    p?.afficher_horaires === true ? 'horaires affichés' : 'horaires masqués',
    p?.afficher_tarifs === true ? 'tarifs affichés' : 'tarifs masqués',
    p?.offres_espace === false ? 'offres hors de l\'espace élève' : 'offres dans l\'espace élève',
    p?.langue_portail === 'en' ? 'en anglais' : p?.langue_portail === 'fr' ? 'en français' : 'langue automatique',
  ].join(' · '),
  // v117 : le lien d'avis Google, écrit par route dédiée (jamais dans CARTES).
  avis:        (p) => {
    const a = p?.avis_google;
    const lien = a && typeof a === 'object' && typeof a.lien === 'string' && a.lien.trim();
    if (!lien) return 'Aucun lien pour l\'instant';
    return a.auto === false ? 'lien posé · email automatique éteint' : 'lien posé · email après la 3e séance';
  },
  page_plus:   (p) => {
    const parts = [];
    if (p?.annees_experience) parts.push(`${parseInt(p.annees_experience)} ans d'expérience`);
    if (p?.formations) parts.push('formations');
    if (p?.philosophie) parts.push('philosophie');
    const faq = Array.isArray(p?.faq_publique) ? p.faq_publique.length : 0;
    if (faq) parts.push(pluriel(faq, 'question', 'questions'));
    return parts.length ? parts.join(' · ') : 'Expérience, formations, philosophie, FAQ';
  },
  docs:        (p) => resumeRubrique(rubriqueParId('documents'), p),
  apparence:   (p) => resumeRubrique(rubriqueParId('types-cours'), p),
  visibilite:  (p) => resumeRubrique(rubriqueParId('visibilite'), p),
  essai:       (p) => resumeRubrique(rubriqueParId('essai'), p),
  paiement:    (p) => resumeRubrique(rubriqueParId('paiement-en-ligne'), p),
  seuils:      (p) => `${parseInt(p?.alerte_seances_seuil) || 2} séances · ${parseInt(p?.alerte_expiration_jours) || 7} jours avant la fin`,
  seuils_prof: (p) => `après ${parseInt(p?.alerte_paiement_attente_jours) || 14} jours d'attente`,
  anniv:       (p) => ((p?.anniversaire_mode || 'semi') !== 'off' ? 'Activé · message prêt' : 'Désactivé'),
  notifs_eleves: (p) => {
    const n = p?.notifs_eleves && typeof p.notifs_eleves === 'object'
      ? Object.values(p.notifs_eleves).filter(v => v && typeof v === 'object' && v.email === true).length : 0;
    return n ? pluriel(n, 'email automatique activé', 'emails automatiques activés') : 'Aucun email automatique';
  },
  annulation:  (p) => resumeRubrique(rubriqueParId('annulation'), p),
  qr:          () => 'Carte de visite, flyer, affiche',
  integrer:    (p) => (p?.couleurs_marque?.c1 ? 'Planning et offres, à tes couleurs' : 'Planning et offres, deux lignes à coller'),
  plan_actuel: (p) => resumeRubrique(rubriqueParId('abonnement'), p),
  changer_plan: (p) => {
    const grille = p?.type_structure === 'association' ? 'Essentiel, Complet, Association' : 'Essentiel, Complet, Studio';
    if (!p) return grille;
    const trial = getTrialStatus(p);
    return trial.active ? 'Choisis ton plan' : grille;
  },
  push:        () => 'Sur cet appareil',
};

export const CARTES_RESUMEES = Object.keys(RESUMES_CARTES);

/** Le résumé d'une carte (fermée), garanti sans exception. */
export function resumeCarte(carte, profile, ctx = {}) {
  try {
    const fn = RESUMES_CARTES[carte];
    const s = fn ? fn(profile, ctx) : '';
    return typeof s === 'string' ? s : '';
  } catch {
    return '';
  }
...
/** Une carte s'ouvre d'elle-même quand elle a quelque chose à demander. */
export function carteOuverteParDefaut(carte, profile) {
  if (carte === 'changer_plan') {
    // Ouverte en essai (choisir est l'action attendue). Essai fini = Essentiel
    // gratuit, une situation stable : on n'ouvre plus la grille d'office.
