 ...
 *   3. la gestion d'erreur : reportError → erreurs_app, jamais d'exception propagée
 *
 * @param {Object} p
 ...
 * @param {Array<{filename:string, content:Buffer|string}>} [p.attachments]  Pièces jointes Resend (v106 : facture PDF)
 ...
 */
export async function sendEmail({ to, subject, html, replyTo = null, from = null, categorie = 'notification', attachments = null }) {
  ...
  if (estEmailDeTest(dest)) {
    console.warn('[email] domaine de test (RFC 2606), envoi ignoré :', dest, '—', subject);
    return { ok: false, skipped: 'domaine_test' };
  }
  ...
  try {
    ...
    const { error } = await resend.emails.send({
      ...
      subject,
      html,
      // ⚠️ Le SDK Resend attend `replyTo` (camelCase) — `reply_to` est
      // silencieusement IGNORÉ (audit 2026-07-25 : « réponds à cet email »
      // partait chez IziSolo au lieu de la prof, sur TOUS les emails).
      ...(replyTo ? { replyTo } : {}),
      // Pièces jointes (v106 : la facture en PDF). Tableau Resend natif
      // [{ filename, content: Buffer }] — passé tel quel, jamais transformé.
    ...
    reportError('[email] envoi échoué :', subject, '—', err);
