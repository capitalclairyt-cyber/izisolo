...
import { localeDe } from './i18n-portail.js';
...
export async function traiterInvoiceEchouee(admin, profileId, invoice) {
  ...
  if (client?.email) {
    ...
    if (claimed) {
      // v122 : l'email à l'ÉLÈVE part dans sa langue (fiche > studio >
      // français), lue à part ; tout ce qui s'adresse à la prof reste en
      // français. Le nom de l'offre et du studio viennent de la base.
      const t = await traducteurEleve(admin, { clientId: client.id, profileId });
      const studio = profil.studio_nom || t('ton studio');
      const montantHtml = `<strong>${t.langue === 'en' ? `€${escapeHtml(String(l.montant))}` : `${escapeHtml(String(l.montant))} €`}</strong>`;
      const prochaine = l.prochaineTentative
        ? new Date(l.prochaineTentative + 'T12:00:00').toLocaleDateString(localeDe(t.langue), { weekday: 'long', day: 'numeric', month: 'long' })
        : null;
      const suite = final
        ? t("Après plusieurs tentatives, il n'a pas abouti : ton abonnement est {pause} le temps de régler ça. Réponds à cet email pour convenir d'un autre règlement avec {studio}, ou mets ta carte à jour depuis l'email de Stripe.", { pause: `<strong>${t('mis en pause')}</strong>`, studio: escapeHtml(studio) })
        : (prochaine
          ? t("Rien de grave : une nouvelle tentative aura lieu le {date}. Si ta carte a changé, mets-la à jour depuis l'email que Stripe t'a envoyé.", { date: `<strong>${escapeHtml(prochaine)}</strong>` })
          : t("Rien de grave : une nouvelle tentative aura lieu dans les prochains jours. Si ta carte a changé, mets-la à jour depuis l'email que Stripe t'a envoyé."));
      const r = await sendEmail({
        to: client.email,
        subject: final
          ? t("Ton prélèvement n'a pas abouti · {studio}", { studio })
          : t('Petit souci de prélèvement · {studio}', { studio }),
        replyTo: profil.email_contact || undefined,
        categorie: 'transactionnel',
        html: `
          <div style="font-family:Inter,Arial,sans-serif;max-width:560px;margin:0 auto;color:#2b2320;line-height:1.55">
            <p>${client.prenom ? t('Bonjour {prenom}', { prenom: escapeHtml(client.prenom) }) : t('Bonjour')},</p>
            <p>${t("Le prélèvement de {montant} pour {offre} chez {studio} n'a pas pu être effectué par ta banque.", { montant: montantHtml, offre: `<strong>${escapeHtml(offreNom)}</strong>`, studio: `<strong>${escapeHtml(studio)}</strong>` })}</p>
            <p>${suite}</p>
            <p style="color:#7a6a63;font-size:13px">${t('Une question ? Réponds simplement à cet email : il arrive chez {studio}.', { studio: escapeHtml(studio) })}</p>
          </div>`,
      });
      notifie = !!r.ok;
    }
