...
//   - Trial 30 jours sur les 2 plans, sans CB
...
export const CAPACITES = {
  ...
  // ── Plusieurs profs dans un même studio (Multi) ──────────────────────
  equipe:               'multi', // inviter des profs, leur donner des droits
...
export const PLANS = {
  ...
  multi: {
    // Association ou studio à plusieurs profs (chantier multi-prof, 2026-08-25).
    // FORFAIT PLAT, profs illimitées : compter les sièges ramènerait la
    // proration Stripe, les quotas et le blocage au 4e membre — tout ce que
    // B3a a justement retiré de l'app.
    nom: 'Multi',
    public: true,
    prix: 49,
    prixAnnuel: 470, // 49 × 12 × 0.8 ≈ 470 € (annuel toujours désactivé)
    limiteClients: null,
    limiteLieux: null,
    limiteOffres: null,
  ...
  multi_free: {
    // La bêta du multi : offerte aux premiers studios qui l'essuient.
    // ⚠️ Ce n'est PAS `free`. `free` ouvre TOUTES les capacités (can() y
    // court-circuite) : une bêta sur `free` testerait un produit qui n'existe
    // pas, et ne remonterait aucune vraie friction. `multi_free` est le plan
    // Multi À L'IDENTIQUE, moins la facture — effectivePlan le rend en 'multi',
    // exactement comme premium → pro. Clé séparée pour pouvoir les COMPTER :
    // un studio bêta n'est pas un compte interne exempté, et il a vocation à
    // devenir payant.
    nom: 'Multi (bêta offerte)',
    public: false,
    prix: 0,
    prixAnnuel: 0,
    limiteClients: null,
    limiteLieux: null,
    limiteOffres: null,
    fraisStripeIziSolo: 1, // même économie que Multi : la bêta doit être fidèle
  },
  premium: {
...
// `multi` y est entré le 2026-09-07 : le script setup-stripe-saas crée son
// Price et le checkout l'accepte (une carte dont le checkout renvoie 500 est
// pire que ne rien afficher, cause racine n°4 « promesse > produit »).
export const PUBLIC_PLANS = ['solo', 'pro', 'multi'];

// Tous les plans valides en DB (free interne, multi_free bêta, premium legacy)
export const ALL_PLANS = ['free', 'solo', 'pro', 'multi', 'multi_free', 'premium'];
