...
export async function countUnread(supabase, viewerKind, viewerId) {
  ...
  // Chemin nominal : agrégat SQL v90 — 1 requête au lieu de 1 PAR conversation
  // (AUDIT-PERF cat 2.1 : ce compteur, pollé par le badge nav pro et le header
  // élève, était le premier poste de charge DB projeté). La RPC est
  // service_role only (SECURITY DEFINER) : le viewer passé en paramètre a déjà
  // été authentifié par l'appelant (withRoute / page serveur).
  try {
    const { createAdminClient } = await import('@/lib/supabase-admin');
    const { data, error } = await createAdminClient().rpc('messages_non_lus_total', {
      p_viewer: viewerKind,
      p_profile_id: viewerKind === 'pro' ? viewerId : null,
      p_client_ids: viewerKind === 'eleve' ? [viewerId] : null,
    });
    if (!error) return Number(data) || 0;
  } catch { /* pré-migration v90 → boucle historique ci-dessous */ }
