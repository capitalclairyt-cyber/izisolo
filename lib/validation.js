...
// Date au format ISO court YYYY-MM-DD (jour calendaire, sans heure).
const zDateIso = z.string().regex(/^\d{4}-\d{2}-\d{2}$/);
...
// Inscription en série à un cours récurrent (portail élève authentifié).
export const reserverSerieSchema = z.object({
  coursId: zUuid,
  jusquAu: zDateIso,
});

// Demande de cours d'essai (portail public). prenom/email requis, le reste
// optionnel ; les champs hors-schéma (website/turnstileToken honeypot, nom,
// telephone, message) restent disponibles via passthrough pour l'anti-bot.
export const essaiSchema = z
  .object({
    coursId: zUuid,
    prenom: trimmedString(120),
    email: zEmail,
  })
  .passthrough();

// Libération en masse des présences futures d'un·e élève sur une récurrence.
export const libererSerieSchema = z.object({
  clientId: zUuid,
  recurrenceId: zUuid,
  depuisDate: zDateIso.optional(),
});
...
// Diffusion d'annonce (mailing groupé) — POST /api/messagerie/announce.
// Valide la forme du body ; l'ownership des ids (cours_id / offre_id /
// shared_ref_id) est vérifié côté route avant de cibler des élèves.
export const messagerieAnnounceSchema = z.object({
  content:            z.string().trim().max(4000).optional(),
  media_urls:         z.array(z.string().trim().max(2000)).max(20).optional(),
  shared_ref_type:    z.enum(['cours', 'offre', 'abonnement']).optional(),
  shared_ref_id:      zUuid.optional(),
  scope:              z.enum(['tous', 'cours', 'type_cours', 'abonnement', 'clients']),
  mode:               z.enum(['individuel', 'groupe']).optional(),
  cours_id:           zUuid.optional(),
  type_cours:         optionalString(120),
  offre_id:           zUuid.optional(),
  client_ids:         z.array(zUuid).max(500).optional(),
  // Décochage avant envoi (2026-08-01) : ids retirés de la liste résolue
  exclure_client_ids: z.array(zUuid).max(500).optional(),
});

// Aperçu des destinataires du composeur (mêmes champs de ciblage, sans contenu)
export const messagerieAnnoncePreviewSchema = z.object({
  scope:      z.enum(['tous', 'cours', 'type_cours', 'abonnement', 'clients']),
  mode:       z.enum(['individuel', 'groupe']).optional(),
  cours_id:   zUuid.optional(),
  type_cours: optionalString(120),
...
// ── Studio concierge (admin) : création + liens ─────────────────────────────
export const adminStudioCreerSchema = z.object({
  prenom: z.string().trim().min(1).max(80),
  email: z.string().trim().toLowerCase().email().max(200),
  studioNom: z.string().trim().min(2).max(120),
  metier: z.enum(['yoga', 'pilates', 'danse', 'musique', 'coaching', 'arts', 'autre']),
});

export const adminStudioCibleSchema = z.object({
  profileId: zUuid,
});

// Archive des déclarations URSSAF (v94). `montant` et `snapshot` sont un
// REPORT de ce que la page serveur a calculé et affiché : ils n'ouvrent aucun
// droit et n'entrent dans aucun calcul, ils ne remplissent que l'archive de la
// prof (bornée par RLS à ses propres lignes).
export const urssafDeclarationSchema = z.object({
  periodeId: z.string().min(1).max(20),
  action: z.enum(['consultee', 'declaree']),
  montant: z.number().min(0).max(100000000),
  snapshot: z.record(z.string(), z.any()).nullable().optional(),
});

// Suppression définitive d'un studio depuis l'admin. `confirmation` = le nom
// du studio RETAPÉ à la main : la vérification exacte vit dans
// lib/admin-suppression.js, ici on borne juste la forme.
export const adminSupprimerStudioSchema = z.object({
  profileId: zUuid,
  confirmation: z.string().min(1).max(200),
  supprimerOrphelins: z.boolean().optional().default(false),
});

export const coursInviterSchema = z.object({
  coursId: zUuid,
});
...
// Aligné avec ALL_PLANS de lib/constantes.js : asso / studio depuis le
// 2026-09-13, multi / multi_free / premium en legacy (posables, traduits par
// effectivePlan). `free` reste accessible pour exempter Colin/Maude.
export const adminUpdatePlanSchema = z.object({
  userId: zUuid,
  plan: z.enum(['free', 'solo', 'pro', 'asso', 'studio', 'multi', 'multi_free', 'premium']),
});

// Import en masse d'élèves (POST /api/clients/import). Le CSV est parsé et
// mappé CÔTÉ CLIENT ; on reçoit un tableau d'objets déjà rangés. Schéma
// VOLONTAIREMENT permissif (champs en string libre, normalisés dans la route)
// pour qu'une cellule douteuse ne fasse jamais échouer tout l'import.
export const clientsImportSchema = z.object({
  clients: z.array(z.object({
    ...
    date_naissance: z.string().max(40).optional(),
    ville:          z.string().max(200).optional(),
    notes:          z.string().max(5000).optional(),
  })).min(1, 'Aucune ligne à importer').max(2000, 'Maximum 2000 élèves par import'),
});
...
const SLUG_BLOCKLIST = new Set([
  ...
  'privacy', 'terms', 'rgpd', 'help', 'docs', 'blog', 'news', 'unsubscribe',
