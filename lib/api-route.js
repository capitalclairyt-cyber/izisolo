...
import { requireCapacite } from './plan-guard';
import { peut } from './studio-membre';
 ...
 * reportError → erreurs_app, et un format d'erreur UNIQUE : { error, code, issues? }.
 *
 * Usage :
 ...
 *   perm      permission de la PERSONNE dans le studio (lib/studio-membre) →
 *             403 { code:'PERMISSION_REQUISE' } si elle ne l'a pas. À ne pas
 *             confondre avec `plan` : `plan` dit ce que le STUDIO a acheté,
 *             `perm` dit ce que CETTE personne a le droit d'y faire. Une prof
 *             invitée sans `argent_gerer` est dans un studio qui a tout payé.
 *             Les deux gardes se cumulent, dans cet ordre.
 *             ⚠️ Une permission qui ne vit que dans l'UI ne vaut RIEN : le
 *             navigateur peut appeler la route à la main.
 *
...
export function withRoute(opts, handler) {
  const { auth = 'user', schema = null, rateLimit = null, plan = null, perm = null } = opts || {};
  ...
  return async function route(request, ctx) {
    try {
      ...
      if (auth === 'cron') {
        ...
        // MFA (2026-08-21) : un compte admin qui a un facteur TOTP VÉRIFIÉ
        // doit présenter une session aal2 ICI AUSSI — sinon le layout exige
        // le code à l'écran mais l'API resterait ouverte au mot de passe
        // seul. Même fail-open que le layout : sans facteur enrôlé ou si
        // l'API MFA répond en erreur, jamais bloquant.
        try {
          const { data: aal } = await authCtx.supabase.auth.mfa.getAuthenticatorAssuranceLevel();
          if (aal?.currentLevel === 'aal1' && aal?.nextLevel === 'aal2') {
            return Response.json({ error: 'Vérification MFA requise', code: 'MFA_REQUIRED' }, { status: 401 });
          }
        } catch { /* fail-open */ }
      }

      // Gate de plan (Sprint 3) : la feature doit être incluse dans le plan
      // EFFECTIF du compte (trial 30 j = pro). 403 + upgradeTo sinon.
      if (plan && authCtx) {
        // `plan:` = une CAPACITÉ (lib/constantes CAPACITES), testée sur le
        // profil déjà chargé par l'auth — plus de re-fetch (B3a).
        const guard = requireCapacite(authCtx.profile, plan);
        if (guard) return guard;
      }

      // Garde de PERMISSION : ce que cette personne a le droit de faire DANS
      // ce studio. Le propriétaire passe toujours ; un membre révoqué, jamais.
      if (perm && authCtx) {
        if (!peut(authCtx.membre, perm)) {
          return Response.json(
            {
              error: "Tu n'as pas les droits pour faire ça dans ce studio.",
              code: 'PERMISSION_REQUISE',
              permission: perm,
            },
            { status: 403 }
          );
