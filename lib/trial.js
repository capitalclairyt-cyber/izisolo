...
export function getAccountStatus(profile) {
  ...
  // Subscription Stripe active = full access
  if (subStatus === 'active' || subStatus === 'trialing') return 'subscribed';

  // Past due = paiement échoué, on garde l'accès le temps que la prof
  // mette à jour sa carte (Stripe re-essaie 3-4 fois avant cancel définitif)
  if (subStatus === 'past_due') return 'past_due';

  // Canceled = abo terminé après cycle complet → compte gelé
  if (subStatus === 'canceled') return 'canceled';

  // Plan pro/premium assigné manuellement (sans Stripe) = accès complet
