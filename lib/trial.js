...
export function getTrialStatus(profile) {
  ...
  const manualPaid = (profile?.plan === 'pro' || profile?.plan === 'premium') && !subStatus;
...
export function getAccountStatus(profile) {
  ...
  // Plan pro/premium assigné à la main (bêta, geste admin), sans Stripe.
  if (profile.plan === 'pro' || profile.plan === 'premium') return 'subscribed';
