...
export function estCompteTest({ email, studio_slug, studio_nom }) {
  ...
  // Studios d'ENTRAÎNEMENT (exercices de formation Maude, 2026-08-21) :
  // créés avec des emails formation-*@example.com — jamais dans les stats.
  // example.com est de toute façon un domaine de test (RFC 2606).
...
/**
 * Ids des profils de TEST (démo Atelier Soleil, melutek, colin+*) — pour
 * filtrer les données BRUTES (cours, clients, paiements, fil d'activité)
 * des pages admin qui ne passent pas par enrichirProfil (2026-08-19,
 * retour Colin : /admin/stats affichait encore les 812 € seedés du démo).
 * Emails chargés au mieux (1 page listUsers) : l'heuristique slug/nom
 * attrape déjà les comptes démo même sans email.
 */
export async function idsComptesTest(admin) {
  const { data: profils } = await admin.from('profiles').select('id, studio_slug, studio_nom');
  const emailById = {};
  try {
    const { data: page } = await admin.auth.admin.listUsers({ page: 1, perPage: 1000 });
    for (const u of page?.users || []) emailById[u.id] = u.email;
  } catch { /* emails indisponibles : slug/nom suffisent pour les démos */ }
  const ids = new Set();
  for (const p of (profils || [])) {
    if (estCompteTest({ email: emailById[p.id], studio_slug: p.studio_slug, studio_nom: p.studio_nom })) {
      ids.add(p.id);
    }
  }
  return ids;
...
// Le prix MENSUEL de chaque plan encaissable (un annuel compte pour un
// douzième : c'est un MRR). Legacy (multi, premium) lus pour les comptes
// historiques. `solo` vaut 0 depuis le freemium.
const PRIX_MENSUEL = Object.fromEntries(
  Object.entries(PLANS).map(([k, p]) => [k, p?.prix ?? 0])
);
 ...
 */
export function enrichirProfil(p, emailById, lastSignInById, usage, acquisitionById = {}) {
  const email = emailById[p.id] || null;
  // Statut ADMIN : « offert » pour un plan posé à la main sans Stripe.
  const statut = getAdminStatus(p);
  const trial = getTrialStatus(p);
  return {
    ...p,
    email,
    last_sign_in_at: lastSignInById[p.id] || null,
    // D'où vient le compte (metadata `acquisition` posée à l'inscription par
    // /register depuis les utm_* de l'annonce ; null = direct ou inconnu).
    acquisition: acquisitionById[p.id] || null,
    compte_statut: statut,               // free | trial_active | gratuit | subscribed | offert | past_due | impaye | canceled
...
export function repartitionStatuts(profilsEnrichis) {
  const rep = { subscribed: 0, offert: 0, trial_active: 0, gratuit: 0, past_due: 0, impaye: 0, canceled: 0, free: 0 };
