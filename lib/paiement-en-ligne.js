...
//   2. déclarer le webhook Stripe dans Paramètres → Argent → Paiement en ligne, pour
...
/**
 * La section « Les offres du studio » de l'ESPACE élève est-elle proposée ?
 * (v108, retour Manon 2026-09-07 : elle vend depuis son site, IziSolo lui
 * parlait par-dessus.) Défaut TRUE : seule une valeur `false` explicite la
 * masque — colonne absente, profil null ou valeur difforme = visible, comme
 * avant. Indépendant de `afficher_tarifs` (la grille PUBLIQUE).
 *
 * @param {{offres_espace?: boolean|null}|null} profile
 */
export function catalogueEspaceVisible(profile) {
  return profile?.offres_espace !== false;
}
