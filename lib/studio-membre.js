...
export const PERMISSIONS = [
  ...
  // v113 (lot 3 Assos & Studios) : les documents de la structure (statuts,
  // PV, contrats) et les assemblées. Tenue par la base (RLS v113).
  { cle: 'documents',    label: 'Gérer les documents',         aide: 'Statuts, PV, contrats, assemblées.', rls: true },
];
 ...
 */
/**
 * Les permissions de LECTURE : les seules qui survivent quand le studio n'a
 * plus le plan qui permet de travailler à plusieurs (lot 1 Associations &
 * Studios, §6.8 : lecture seule, jamais une révocation silencieuse).
 */
export const PERMISSIONS_LECTURE = ['eleves_voir', 'argent_voir'];

export function peut(membre, permission) {
  if (!membre || membre.statut !== 'actif') return false;
  if (membre.role === 'proprietaire') return true;
  // Studio sans plan d'équipe : la membre entre, lit, et ne modifie rien.
  // Posé par la résolution du studio actif (lib/studio-actif), jamais en base.
  if (membre.lecture_seule === true && !PERMISSIONS_LECTURE.includes(permission)) return false;
...
export function sanitizePortee(v) {
  return v === 'miens' ? 'miens' : PORTEE_POINTAGE_DEFAUT;
}

export function labelPortee(v) {
  return v === 'miens' ? 'Seulement ses séances' : 'Toutes les séances';
}

/**
 * Cette personne peut-elle pointer CETTE séance ? Miroir EXACT de la fonction
 * SQL `mes_cours_pointables()` (v103), qui est la vraie garde : ici on ne fait
 * qu'éviter d'ouvrir un écran qui refuserait d'enregistrer.
 *
 * Trois façons de dire oui :
 *   • le propriétaire, ou une portée « tous » ;
 *   • la séance n'a pas d'intervenante désignée (on ne ferme jamais
 *     rétroactivement une porte qui était ouverte) ;
 *   • c'est SA séance.
 *
 * @param {object} membre  son appartenance au studio affiché
 * @param {object} cours   la séance ({ intervenant_id })
 */
export function peutPointerCours(membre, cours) {
  if (!estMembreActif(membre)) return false;
  if (!peut(membre, 'pointer')) return false;
  if (membre.role === 'proprietaire') return true;
  if (sanitizePortee(membre.portee_pointage) === 'tous') return true;
  const intervenant = cours?.intervenant_id;
  if (!intervenant) return true;
  return intervenant === membre.id;
}
