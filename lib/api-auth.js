 ...
 *   • compte 'trial_active' (trial 30 j en cours)
...
export async function requireActiveAccount() {
  ...
  // Depuis le freemium (2026-09-13), UN seul cas gèle : l'impayé. La fin
  // d'essai et la résiliation ramènent sur Essentiel gratuit, sans rien
  // refuser (lib/trial.js).
  if (isAccountFrozen(profile)) {
    const status = getAccountStatus(profile);
    const message = 'Ton abonnement IziSolo a un impayé. Mets ta carte à jour pour continuer à créer.';
    ...
    throw new Response(
      JSON.stringify({
        ...
        code: 'account_frozen',
        status,
        action_required: '/parametres/abonnement',
      }),
