...
export async function enablePush() {
  ...
  try {
    ...
    // ready se résout dès que le SW enregistré par RegisterSW est actif
    // (prouvé < 1 s) — borné quand même : un register raté ne doit pas
    // laisser le bouton en « Activation… » pour toujours.
    const reg = await Promise.race([
      navigator.serviceWorker.ready,
      new Promise((_, rej) => setTimeout(() => rej(new Error('service worker inactif')), 15000)),
    ]);
