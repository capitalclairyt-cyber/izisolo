...
// ────────────────────────────────────────────────────────────────────────────
// Un versement REÇU pendant qu'une ligne attend déjà (2026-09-15, Marie-Pierre
// bis). La veille, l'encart de « Encaisser un versement » CONSEILLAIT
// d'encaisser la ligne de 480 € plutôt que d'ajouter 245 € ; Maude a fait le
// geste naturel (« Déjà reçu », 245 € par chèque) et la fiche a additionné
// 480 + 245 : « reçu 245 € · 480 € restant ». Un conseil ne corrige pas un
// modèle : un versement reçu sur un abo qui a des lignes en attente EST un
// acompte sur ces lignes. Il s'IMPUTE, dans l'ordre des échéances : une ligne
// entièrement couverte devient réglée, la dernière est SCINDÉE (une part
// réglée, le reste toujours attendu). Rien n'est ajouté par-dessus.
// ────────────────────────────────────────────────────────────────────────────

/**
 * Plan d'imputation d'un versement reçu sur les lignes en attente d'un abo.
 * `lignes` = { id, montant, date, statut } en attente (pending / overdue),
 * `montant` = ce qui est arrivé. Rend :
 *   { ok, erreur, actions: [{ id, action: 'regler' } | { id, action: 'scinder',
 *     paye, reste }], surplus }
 * - ordre = date d'échéance croissante (la plus ancienne d'abord) ;
 * - un versement qui DÉPASSE le total attendu est refusé : on n'invente pas
 *   une ligne pour le surplus, la prof saisit un vrai montant ou encaisse les
 *   lignes puis ajoute le complément comme versement sur un abo à jour ;
 * - sans ligne en attente : aucune action, le versement est une ligne neuve
 *   (c'est à l'appelant de la créer).
 */
export function planImputation(lignes, montant) {
  const m = arrondi(Number(montant));
  if (!Number.isFinite(m) || m <= 0) return { ok: false, erreur: 'Montant du versement manquant ou nul.', actions: [], surplus: 0 };
  const dues = (lignes || [])
    .filter(l => l && (l.statut === 'pending' || l.statut === 'overdue') && Number(l.montant) > 0)
    .map(l => ({ id: l.id, montant: arrondi(Number(l.montant)), date: l.date || '' }))
    .sort((a, b) => a.date.localeCompare(b.date));
  if (dues.length === 0) return { ok: true, erreur: null, actions: [], surplus: m };

  const totalDu = arrondi(dues.reduce((s, l) => s + l.montant, 0));
  if (m > totalDu + TOLERANCE) {
    const fmt = n => n.toFixed(2).replace(/\.00$/, '').replace('.', ',');
    return {
      ok: false,
      erreur: `Ce versement (${fmt(m)} €) dépasse ce qui reste attendu sur cet abonnement (${fmt(totalDu)} €). Encaisse d'abord ce qui est attendu, puis ajoute le complément comme versement.`,
      actions: [],
      surplus: arrondi(m - totalDu),
    };
  }
  ...
  let reste = m;
  for (const l of dues) {
    if (reste <= TOLERANCE) break;
    if (reste + TOLERANCE >= l.montant) {
      actions.push({ id: l.id, action: 'regler' });
      reste = arrondi(reste - l.montant);
    } else {
      actions.push({ id: l.id, action: 'scinder', paye: reste, reste: arrondi(l.montant - reste) });
      reste = 0;
    }
  }
  return { ok: true, erreur: null, actions, surplus: 0 };
}

/**
 * La phrase que la modale affiche pendant la saisie, pour que la prof sache
 * ce qui va se passer AVANT de valider. `null` sans ligne en attente.
 */
export function texteImputation(lignes, montant) {
  const dues = (lignes || []).filter(l => l && (l.statut === 'pending' || l.statut === 'overdue') && Number(l.montant) > 0);
  if (dues.length === 0) return null;
  const fmt = n => `${Number(n).toFixed(2).replace(/\.00$/, '').replace('.', ',')} €`;
  const totalDu = arrondi(dues.reduce((s, l) => s + Number(l.montant), 0));
  const m = arrondi(Number(montant));
  if (!Number.isFinite(m) || m <= 0) return `${fmt(totalDu)} attendent sur cet abonnement : ce versement s'en déduira.`;
  const plan = planImputation(dues, m);
  if (!plan.ok) return plan.erreur;
  const apres = arrondi(totalDu - m);
  if (apres <= TOLERANCE) return `Ces ${fmt(m)} soldent les ${fmt(totalDu)} attendus : plus rien à encaisser sur cet abonnement.`;
  return `Ces ${fmt(m)} se déduisent des ${fmt(totalDu)} attendus : il restera ${fmt(apres)} à encaisser.`;
}
