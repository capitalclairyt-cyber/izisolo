...
import { etatLienIntervenante } from './lien-intervenante';
import { sanitizeRemuneration } from './remuneration';
import { sanitizeFonction } from './vie-asso';
...
export function emailInvitation({ studioNom, prenomInvitee, prenomProprietaire, lien, membre, compteExistant }) {
  ...
  return {
    ...
    html: `
      ...
      <p style="color:#6b5f5a;font-size:14px;">Tu donnes aussi des cours à ton compte ? Ton propre IziSolo est <strong>gratuit</strong>, avec la même adresse : tes élèves, ton agenda, tes encaissements, et ce studio à côté.</p>`}
...
/**
 * Pose la portée de pointage d'un membre. UPDATE SÉPARÉ, jamais dans l'insert
 * de l'invitation : PostgREST refuse la requête ENTIÈRE quand elle nomme une
 * colonne absente (PGRST204), et c'est l'invitation complète qui serait perdue
 * tant que v103 n'est pas appliquée. Le piège de §12, dans lequel je suis
 * tombé en écrivant ce lot — d'où ce helper.
 *
 * Jamais bloquant : sans la colonne, tout le monde pointe tout, c'est-à-dire
 * exactement le comportement d'avant.
 */
export async function poserPortee(supabase, membreId, portee) {
  if (!membreId) return { ok: true };
  try {
    const { error } = await supabase
      .from('studio_membres')
      .update({ portee_pointage: portee === 'miens' ? 'miens' : 'tous' })
      .eq('id', membreId);
    if (error) {
      const absente = ['PGRST204', '42703'].includes(error.code);
      return { ok: false, migrationManquante: absente };
    }
    return { ok: true };
  } catch {
...
/**
 * Pose prénom et nom d'un membre (v111). UPDATE SÉPARÉ, mêmes raisons que
 * `poserPortee` : nommer une colonne absente dans l'insert de l'invitation
 * ferait perdre l'invitation entière tant que v111 n'est pas appliquée.
 * Jamais bloquant : sans les colonnes, le membre s'affiche par son email.
 */
export async function poserIdentite(supabase, membreId, { prenom, nom }) {
  if (!membreId) return { ok: true };
  const p = String(prenom || '').trim().slice(0, 60) || null;
  const n = String(nom || '').trim().slice(0, 80) || null;
  if (!p && !n) return { ok: true };
  try {
    const { error } = await supabase.from('studio_membres').update({ prenom: p, nom: n }).eq('id', membreId);
    if (error) return { ok: false, migrationManquante: ['PGRST204', '42703'].includes(error.code) };
    return { ok: true };
  } catch {
    return { ok: false, migrationManquante: false };
  }
...
/**
 * Pose la fonction du bureau d'un membre (v113). UPDATE SÉPARÉ, mêmes raisons
 * que poserPortee : la colonne n'existe qu'avec v113.
 */
export async function poserFonction(supabase, membreId, fonction) {
  if (!membreId) return { ok: true };
  try {
    const { error } = await supabase.from('studio_membres').update({ fonction: sanitizeFonction(fonction) }).eq('id', membreId);
    if (error) return { ok: false, migrationManquante: ['PGRST204', '42703'].includes(error.code) };
    return { ok: true };
  } catch {
    return { ok: false, migrationManquante: false };
...
export function membrePublic(m) {
  return {
    id: m.id,
    email: m.email,
    // v111 : la personne (prénom, nom, bio, photo) et son lien permanent.
    // Pré-migration ces colonnes n'existent pas : null, et 'aucun'.
    prenom: m.prenom || null,
    nom: m.nom || null,
    bio: m.bio || null,
    photo_url: m.photo_url || null,
    lien: etatLienIntervenante(m),
    // v112 : ce que la structure a convenu avec elle (null = rien).
    remuneration: sanitizeRemuneration(m.remuneration),
    // v113 : la fonction du bureau (étiquette), null pour un studio.
    fonction: sanitizeFonction(m.fonction),
    lien_expire_at: m.lien_expire_at || null,
    lien_usages: m.lien_usages || 0,
    lien_derniere_utilisation_at: m.lien_derniere_utilisation_at || null,
    role: m.role,
    permissions: m.permissions || {},
    statut: m.statut,
    // Pré-v103 la colonne n'existe pas : 'tous' est le comportement d'avant.
    portee_pointage: m.portee_pointage === 'miens' ? 'miens' : 'tous',
