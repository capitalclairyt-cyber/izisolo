...
export async function sendPortailMagicLink({ email, studioSlug, studioNom, prenom, profPrenom }) {
  ...
  // Le studioSlug est dans le PATH (pas en query) pour qu'il ne puisse pas être
  // perdu par Supabase. /p/[slug]/connecte échange le code puis redirige vers
  // /p/[slug]/espace. Évite le fallback /onboarding quand le `next` se perd.
  const redirectTo = `${appUrl}/p/${studioSlug}/connecte`;
