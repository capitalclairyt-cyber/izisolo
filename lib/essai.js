 ...
 */
export async function emailConfirmationVisiteur({ profileNom, studioSlug, prenom, email, cours, paiement, prix, stripeLink, proEmail = null, adresse = null, codePostal = null, ville = null, telephone = null }) {
  ...
  try {
    ...
    // Bloc "infos pratiques" : adresse + tél + email du studio, et lien vers la
    // page publique (FAQ, accès, parking, matériel à prévoir…).
    const adresseLigne = [adresse, [codePostal, ville].filter(Boolean).join(' ').trim()].filter(Boolean).join(', ');
    const infosLignes = [
      adresseLigne ? `<br/><span style="color:#888;">📍 ${adresseLigne}</span>` : '',
      telephone ? `<br/><span style="color:#888;">📞 ${telephone}</span>` : '',
      proEmail ? `<br/><span style="color:#888;">✉️ ${proEmail}</span>` : '',
    ].join('');
    const infosBlock =
      (infosLignes
        ? `<div style="background:#faf8f5;border-radius:12px;padding:16px 20px;margin:0 0 16px;"><strong style="color:#1a1a2e;">Infos pratiques</strong>${infosLignes}</div>`
        : '') +
      `<p style="color:#555;font-size:0.9rem;margin:0 0 16px;">Accès, parking, matériel à prévoir… retrouve toutes les infos sur <a href="${appUrl}/p/${studioSlug}" style="color:#d4a0a0;font-weight:600;">la page de ${profileNom}</a>.</p>`;
 ...
 */
export async function emailEnAttenteVisiteur({ profileNom, prenom, email, cours, proEmail = null }) {
  if (!process.env.RESEND_API_KEY) return;
  try {
    const resend = new Resend(process.env.RESEND_API_KEY);
    const dateStr = new Date(cours.date + 'T12:00:00').toLocaleDateString('fr-FR', {
