 ...
 *
 * Cours « payable à la séance » (v70) : si `cours.tarif_unitaire` est
 * renseigné (> 0), AUCUN carnet n'est résolu — promesse du formulaire de
 * cours (« il ne décomptera aucun carnet ») ; l'élève règle à la séance.
 * Un carnet DÉJÀ lié à la présence (override explicite) n'est pas concerné :
 * ce cas est géré en amont par l'appelant (`presence.abonnement_id` prioritaire).
 *
 * Cours MIXTE (v82, MODELE-COURS-CARNETS-2026.md R1) : si le cours porte EN
 * PLUS `carnets_acceptes = true`, le tarif devient un FILET — les carnets
 * applicables se résolvent normalement, seuls les élèves non couvertes
 * paient à la séance. ⚠️ Tout appelant qui passe un cours à tarif doit donc
 * inclure `carnets_acceptes` dans son select : colonne absente = undefined =
 * atelier pur (dégradation sûre, mais la promesse mixte serait muette).
 *
 ...
 * @param {{type_cours?: string|null, date?: string|null, tarif_unitaire?: number|null}} cours
 * @returns {object|null} le carnet applicable, ou null si aucun.
 */
export function resoudreCarnetApplicable(abos, cours) {
  // Cours payable à la séance → pas de résolution automatique (v70)…
  // sauf cours MIXTE (v82) : carnets_acceptes = true → le tarif n'est qu'un
  // filet pour les élèves sans carnet applicable.
  if (cours && Number(cours.tarif_unitaire) > 0 && cours.carnets_acceptes !== true) return null;
