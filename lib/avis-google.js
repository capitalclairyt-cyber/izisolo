 ...
 * `t` (v122) : le traducteur de l'élève (lib/i18n-portail) ; par défaut le
 * français, donc chaque appelant existant rend exactement le texte d'avant.
 */
export function emailAvis({ prenom, studioNom, lien, t = traducteur('fr') }) {
  const salut = prenom ? `${t('Bonjour {prenom}', { prenom })},` : `${t('Bonjour')},`;
  return {
    sujet: t('Un mot sur tes séances chez {studio} ?', { studio: studioNom }),
    corps:
`${salut}

${t('Tu es venue plusieurs fois maintenant, et ça fait vraiment plaisir.')}

${t("Si tu as une minute, un avis sur Google aide énormément : c'est comme ça que d'autres personnes trouvent {studio}.", { studio: studioNom })}

${t('Laisser un avis :')} ${lien}

${t("Tu écris ce que tu veux, ce que tu penses vraiment. Ce message n'est envoyé qu'une seule fois.")}

${t('Merci, et à bientôt sur le tapis,')}`,
  };
}
