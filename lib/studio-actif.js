...
export async function resoudreStudioActif(supabase, user) {
  ...
  if (membres.length > 0) {
    ...
    // Chaque appartenance chez autrui est annotée : couverte, ou en LECTURE
    // SEULE (lot 1 Assos & Studios, §6.8, décision Colin). Elle entre, elle
    // lit, elle ne modifie rien : `peut()` refuse toute permission d'écriture
    // à un membre marqué `lecture_seule`, le layout le dit dans un bandeau, et
    // personne n'est révoqué en silence : le jour où la structure re-souscrit,
    // tout revient. Son propre studio n'est jamais en lecture seule.
    ...
    //
    // ⚠️ Trouvé par la preuve du lot 1 en phase complète (2026-09-13) : on ne
    // gardait que les appartenances couvertes, donc une prof membre de DEUX
    // structures dont l'une en lecture seule était renvoyée vers l'autre au
    // lieu de celle qu'elle avait CHOISIE (404 sur ses séances au lieu du 403
    // attendu). Le choix explicite prime, quel que soit le plan de la
    // structure choisie ; sans choix, une structure couverte passe devant.
    const annotees = membres.map(m => (
      m.profile_id === user.id || couverts.has(m.profile_id) ? m : { ...m, lecture_seule: true }
    ));
    const ordonnees = (choisi && annotees[0]?.profile_id === choisi)
      ? annotees
      : [...annotees.filter(m => !m.lecture_seule), ...annotees.filter(m => m.lecture_seule)];
    const premier = ordonnees[0];
    return { studioId: premier.profile_id, membre: premier, membres: ordonnees, equipeSuspendue: premier.lecture_seule === true };
  }
