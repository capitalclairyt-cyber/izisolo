...
function checkRateLimit(ip, { max = RATE_LIMIT_MAX, windowSec = RATE_LIMIT_WINDOW, scope = 'default' } = {}) {
 ...
 */
export function checkRateLimitIP(request, { max, windowSeconds, scope } = {}) {
...
export async function checkAntiBot(request, opts = {}) {
  ...
  const rl = checkRateLimit(ip, { max: opts.max, windowSec: opts.windowSeconds, scope: opts.scope });
  if (!rl.ok) return rl;
