...
// Plan effectif d'un profile (fallback `solo` si rien — le default à
// l'inscription. JAMAIS `free` par défaut : `free` est uniquement attribué
// par l'admin pour des comptes exemptés).
export function effectivePlan(profile) {
  return PLANS[profile?.plan || 'solo'] ? (profile?.plan || 'solo') : 'solo';
}
...
export async function verifierLimite(supabase, profileId, action) {
  ...
  const planKey = profile?.plan || 'solo';
  const plan = planConfig(planKey);
