  ...
  switch (action) {
    ...
    case 'ajouter_lieu': {
      ...
      if ((count || 0) >= plan.limiteLieux) {
        return {
          allowed: false,
          message: `Tu as atteint la limite de ${plan.limiteLieux} ${plan.limiteLieux > 1 ? 'lieux' : 'lieu'} du plan ${plan.nom}. ${plan.limiteLieux === 1 ? 'Passe en Pro pour gérer jusqu\'à 3 lieux.' : 'Passe en Studio pour des lieux illimités.'}`,
    ...
    case 'ajouter_offre': {
      if (plan.limiteOffres == null) return { allowed: true };
      const { count } = await supabase
        .from('offres')
        .select('*', { count: 'exact', head: true })
        .eq('profile_id', profileId);
      if ((count || 0) >= plan.limiteOffres) {
        return {
          allowed: false,
          message: `Tu as atteint la limite de ${plan.limiteOffres} formules du plan ${plan.nom}. Passe en Pro pour créer des formules illimitées !`,
          upgradeTo: 'pro',
          current: count,
          limit: plan.limiteOffres,
        };
      }
      return { allowed: true, current: count, limit: plan.limiteOffres };
    }
...
// ─── Gate portail public : feature du STUDIO (pas de l'appelant) ───────────

/**
 * Le studio (row profiles avec plan, trial_started_at,
 * stripe_subscription_status) inclut-il cette feature dans son plan effectif ?
 * Utilisé par les routes portail public : un élève ne peut utiliser
 * liste d'attente / essai / annulation en ligne que si le studio les a.
 */
export function studioHasFeature(profileRow, featureKey) {
  return planConfig(effectivePlan(profileRow))[featureKey] === true;
}
