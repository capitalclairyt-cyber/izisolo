...
// ── « Je déclare à part » (v95) ─────────────────────────────────────────────

/**
 * Les encaissements que la prof a sortis de sa déclaration.
 *
 * Lecture SÉPARÉE et défensive, comme essai_tarif ou derniere_activite_at :
 * `exclu_compta` naît avec v95 et ne doit JAMAIS entrer dans le select
 * principal d'un document comptable. Une colonne absente fait échouer TOUTE
 * la requête (42703) — le récap sortirait vide ou faux, sans un mot. Sans la
 * colonne : aucune exclusion, exactement le comportement d'avant v95.
 *
 * @param supabase  client scopé au studio (RLS) ou admin
 * @param profileId studio
 * @param {{from?: string, to?: string}} periode  bornes comptables (facultatif)
 * @returns {Promise<{ids: Set<string>, nb: number, montant: number, disponible: boolean}>}
 */
export async function lireExclusions(supabase, profileId, { from, to } = {}) {
  try {
    let q = supabase
      .from('paiements')
      .select('id, montant')
    ...
    if (from && to) q = q.or(filtreDateComptable(from, to));
    // 5000 : une prof qui exclurait davantage a un autre problème que ce
    // plafond, et le document dirait alors moins que la vérité — on préfère
    // une borne haute et explicite à une troncature invisible à 1000.
    const { data, error } = await q.limit(5000);
    if (error) throw error;
    const lignes = data || [];
    return {
      ids: new Set(lignes.map(p => p.id)),
      nb: lignes.length,
      montant: Math.round(lignes.reduce((s, p) => s + (parseFloat(p.montant) || 0), 0) * 100) / 100,
      disponible: true,
    };
  } catch {
    return { ids: new Set(), nb: 0, montant: 0, disponible: false };
  }
}

/**
 * La phrase qu'un document DOIT porter quand il a écarté des lignes.
 * Un registre qui se tait sur ce qu'il ne contient pas est un faux ; celui-ci
 * annonce le nombre et le montant, et laisse la responsabilité où elle est.
 */
export function mentionExclusions({ nb = 0, montant = 0 } = {}) {
  if (!nb) return '';
  return `${nb} encaissement${nb > 1 ? 's' : ''} volontairement exclu${nb > 1 ? 's' : ''} `
    + `de ce document (« je déclare à part ») : ${montantFr(montant)} €.`;
}

/** Retire d'un lot les paiements exclus. Une seule façon de le faire. */
export function retirerExclus(paiements, exclusions) {
  const ids = exclusions?.ids;
  if (!ids || ids.size === 0) return paiements || [];
  return (paiements || []).filter(p => !ids.has(p.id));
}
